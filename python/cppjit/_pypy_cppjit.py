...
# pull in the built-in low-level cppjit
try:
    c = loader.load_cpp_backend()
except RuntimeError:
    import sysconfig
    os.environ['CPPJIT_BACKEND_LIBRARY'] = "libcppjit_backend"+sysconfig.get_config_var("SO")
    c = loader.load_cpp_backend()
os.environ['CPPJIT_BACKEND_LIBRARY'] = c._name

# some older versions can be fixed up through a compatibility
# module on the python side; load it, if available
try:
    import cppjit_compat
except ImportError:
    pass

import _cppjit as _backend     # built-in module
try:
