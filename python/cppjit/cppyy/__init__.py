...
if not 'CLING_STANDARD_PCH' in os.environ:
    def _set_pch():
        try:
            from .. import cppyy_backend as cpb
...
if not ispypy:
    ...
    if apipath_extra is None:
        # for the monorepo: headers are at cppjit/cppyy_backend/include/
        _cppjit_inc = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'cppyy_backend', 'include')
        if os.path.exists(os.path.join(_cppjit_inc, 'CPyCppyy')):
