""" cppjit extensions for numba
"""

import cppjit
import cppjit.types as cpp_types
import cppjit.reflex as cpp_refl
...
ir_intptr_t = ir.IntType(cppjit.sizeof('void*')*8)

# special case access to unboxing/boxing APIs
cppjit_as_voidptr   = cppjit.addressof('Instance_AsVoidPtr')
cppjit_from_voidptr = cppjit.addressof('Instance_FromVoidPtr')
...
def typeof_scope(val, c, q = Qualified.default):
  ...
  # create a model description for Numba
    if q == Qualified.default:
        ...
        def init(self, dmm, fe_type, sz = cppjit.sizeof(val)):
            self._data_members = data_members
            self._member_methods = member_methods
            self._sizeof = sz
  ...
  # Python proxy unwrapping for arguments into the Numba trace
    ...
    def unbox_instance(typ, obj, c):
        global cppjit_as_voidptr

        ptrty = ir.PointerType(ir.FunctionType(ir_voidptr, [ir_voidptr]))
        ptrval = c.context.add_dynamic_addr(c.builder, cppjit_as_voidptr, info='Instance_AsVoidPtr')
