...
""" cppjit_backend loader
"""

__all__ = [
    'load_cpp_backend',           # load libcppjit_backend
...
def load_cpp_backend():
    ...
    names = list()
    try:
        bkname = os.environ['CPPJIT_BACKEND_LIBRARY']
        if bkname.rfind(soext) < 0:
            bkname += soext
        names.append(bkname)
    except KeyError:
        names.append('libcppjit-backend'+soext)
        if soabi:
            names.append('libcppjit-backend.'+soabi+soext)
        if soext2:
            names.append('libcppjit-backend'+soext2)
    ...
    if not c:
        raise RuntimeError("could not load cppjit_backend library, details:\n%s" %
            '\n'.join(['  '+x for x in err]))
