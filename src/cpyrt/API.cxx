...
//-----------------------------------------------------------------------------
PyObject* cpyrt::Instance_FromVoidPtr(
...
// perform cast (the call will check TClass and addr, and set python errors)
    PyObject* pyobject = BindCppObjectNoCast(addr, interop::GetScope(classname), false);
...
//-----------------------------------------------------------------------------
PyObject* cpyrt::Instance_FromVoidPtr(
    void* addr, interop::TCppScope_t klass_scope, bool python_owns)
{
// Bind the addr to a python object of class defined by classname.
    if (!Initialize())
        return nullptr;
...
//-----------------------------------------------------------------------------
void cpyrt::ExecScript(const std::string& name, const std::vector<std::string>& args)
{
