...
namespace cppjit::cpyrt {
    ...
    PyObject* Instance_FromVoidPtr(void* addr, interop::TCppScope_t klass_scope, bool python_owns);
}


//- protected members --------------------------------------------------------
bool cpyrt::CPPConstructor::InitExecutor_(Executor*& executor, CallContext*)
{
...
//- public members -----------------------------------------------------------
PyObject* cpyrt::CPPConstructor::GetDocString()
{
// GetMethod() may return an empty function if this is just a special case place holder
    ...
    return cpyrt_PyText_FromFormat("%s::%s%s",
        clName.c_str(), clName.c_str(), this->GetMethod() ? this->GetSignatureString().c_str() : "()");
}

//----------------------------------------------------------------------------
PyObject* cpyrt::CPPConstructor::Reflex(
    interop::Reflex::RequestId_t request, interop::Reflex::FormatId_t format)
{
// C++ reflection tooling for constructors.

    if (request == interop::Reflex::RETURN_TYPE) {
        std::string fn = interop::GetScopedFinalName(this->GetScope());
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPConstructor::Call(CPPInstance*& self,
...
// conversion context. See also the ConvertImplicit() helper in Converters.cxx.
    if((cppScopeFlags & CPPScope::kActiveImplicitCall) && interop::IsExplicit(GetMethod())) {
        // FIXME: Cases with explicit marked std::complex constructors where we expect implicit conversionss
        if (interop::GetMethodSignature(GetMethod(), true).find("std::complex") == std::string::npos)
            return nullptr;
    }
...
// perform the call, nullptr 'this' makes the other side allocate the memory
    interop::TCppScope_t disp = self->ObjectIsA(false /* check_smart */);
    intptr_t address = 0;
    if (GetScope() != disp) {
    ...
    // get the dispatcher class and verify
        ...
        if (!(((CPPClass*)dispproxy)->fFlags & CPPScope::kIsPython)) {
            PyErr_SetString(PyExc_TypeError, const_cast<char*>((
                "constructor for " + interop::GetScopedFinalName(disp) + " is not a dispatcher").c_str()));
            return nullptr;
        }

        PyObject* pyobj = cpyrt_PyObject_Call(dispproxy, cargs.fArgs, cargs.fNArgsf, kwds);
        if (!pyobj)
            return nullptr;
...
// return object if successful, lament if not
    ...
    // mark as actual to prevent needless auto-casting and register on its class
        ...
        if (!(((CPPClass*)Py_TYPE(self))->fFlags & CPPScope::kIsSmart))
            MemoryRegulator::RegisterPyObject(self, interop::TCppObject_t((void*)address));
    ...
    if (!PyErr_Occurred())   // should be set, otherwise write a generic error msg
        PyErr_SetString(PyExc_TypeError, const_cast<char*>(
            (interop::GetScopedFinalName(GetScope()) + " constructor failed").c_str()));
...
//----------------------------------------------------------------------------
cpyrt::CPPMultiConstructor::CPPMultiConstructor(interop::TCppScope_t scope, interop::TCppMethod_t method) :
    CPPConstructor(scope, method)
{
    fNumBases = interop::GetNumBases(scope);
}

//----------------------------------------------------------------------------
cpyrt::CPPMultiConstructor::CPPMultiConstructor(const CPPMultiConstructor& s) :
    CPPConstructor(s), fNumBases(s.fNumBases)
{
}

//----------------------------------------------------------------------------
cpyrt::CPPMultiConstructor& cpyrt::CPPMultiConstructor::operator=(const CPPMultiConstructor& s)
{
    if (this != &s) {
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPMultiConstructor::Call(CPPInstance*& self,
    cpyrt_PyArgs_t argsin, size_t nargsf, PyObject* kwds, CallContext* ctxt)
{
...
// now unroll the new args tuple into a vector of objects
    ...
    cpyrt_PyArgs_t _args = argsu.get();
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPAbstractClassConstructor::Call(CPPInstance*& self,
    cpyrt_PyArgs_t args, size_t nargsf, PyObject* kwds, CallContext* ctxt)
{
// do not allow instantiation of abstract classes
    if ((self && GetScope() != self->ObjectIsA()
        ) || (!self && !(ctxt->fFlags & CallContext::kFromDescr) && \
              cpyrt_PyArgs_GET_SIZE(args, nargsf) && CPPInstance_Check(args[0]) && \
    ...
    PyErr_Format(PyExc_TypeError, "cannot instantiate abstract class \'%s\'"
        ...
        interop::GetScopedFinalName(this->GetScope()).c_str());
    return nullptr;
}


//----------------------------------------------------------------------------
PyObject* cpyrt::CPPNamespaceConstructor::Call(
    CPPInstance*&, cpyrt_PyArgs_t, size_t, PyObject*, CallContext*)
{
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPIncompleteClassConstructor::Call(
    CPPInstance*&, cpyrt_PyArgs_t, size_t, PyObject*, CallContext*)
{
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPAllPrivateClassConstructor::Call(
    CPPInstance*&, cpyrt_PyArgs_t, size_t, PyObject*, CallContext*)
{
