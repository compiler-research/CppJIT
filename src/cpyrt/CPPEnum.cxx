...
static PyObject* enum_repr(PyObject* self)
{
    ...
    if (kls_cppname && obj_cppname && obj_str) {
        const std::string resolved = interop::ResolveEnum(PyLong_AsVoidPtr(kls_scope));
        repr = cpyrt_PyText_FromFormat("(%s::%s) : (%s) %s",
            cpyrt_PyText_AsString(kls_cppname), cpyrt_PyText_AsString(obj_cppname),
            resolved.c_str(), cpyrt_PyText_AsString(obj_str));
    }
...
//- creation -----------------------------------------------------------------
cpyrt::CPPEnum* cpyrt::CPPEnum_New(const std::string& name, interop::TCppScope_t scope)
{
...
// the type but may also live in the enclosing scope.
    ...
    // add the __module__ to allow pickling
        ...
        PyObject* pymodname = cpyrt_PyText_FromString(("cppjit.gbl"+modname).c_str());
    ...
    // collect the enum values
        std::vector<interop::TCppScope_t> econstants = interop::GetEnumConstants(etype);
        bool values_ok = true;
        for (auto econstant : econstants) {
            ...
            Py_DECREF(pydname);
            PyObject* pydcppname = cpyrt_PyText_FromString((ename.empty() ? dname : (ename+"::"+dname)).c_str());
