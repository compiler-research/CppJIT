...
//----------------------------------------------------------------------------
PyObject* cpyrt::pyval_from_enum(const std::string& enum_type, PyObject* pytype,
                                 PyObject* btype,
                                 interop::TCppScope_t enum_constant) {
  long long llval = interop::GetEnumDataValue(enum_constant);
...
//- enum methods -------------------------------------------------------------
static int enum_setattro(PyObject* /* pyclass */, PyObject* /* pyname */,
                         PyObject* /* pyval */) {
...
static PyObject* enum_repr(PyObject* self) {
  ...
  PyObject* kls_scope =
      PyObject_GetAttr((PyObject*)Py_TYPE(self), PyStrings::gThisModule);
  if (!kls_scope)
    PyErr_Clear();
  PyObject* kls_cppname =
      PyObject_GetAttr((PyObject*)Py_TYPE(self), PyStrings::gCppName);
  if (!kls_cppname)
    PyErr_Clear();
...
//----------------------------------------------------------------------------
// TODO: factor the following lookup with similar codes in Convertes and
// TemplateProxy.cxx
...
static PyTypeObject* GetCTypesType(const std::string& cppname) {
  ...
  if (nn == gCTypesNames.end()) {
    PyErr_Format(PyExc_TypeError, "Can not find ctypes type for \"%s\"",
                 cppname.c_str());
    return nullptr;
  }
...
//- creation -----------------------------------------------------------------
cpyrt::CPPEnum* cpyrt::CPPEnum_New(const std::string& name,
                                   interop::TCppScope_t scope) {
  // Create a new enum type based on the actual C++ type. Enum values are added
  // to the type but may also live in the enclosing scope.

  CPPEnum* pyenum = nullptr;

  interop::TCppScope_t etype = scope;
  const std::string& ename = interop::GetScopedFinalName(scope);
  if (etype) {
    // create new enum type with labeled values in place, with a meta-class
    // to make sure the enum values are read-only
    const std::string& resolved = interop::ResolveEnum(etype);
    ...
    std::vector<interop::TCppScope_t> econstants =
        interop::GetEnumConstants(etype);
    bool values_ok = true;
