...
void* cpyrt::CPPInstance::GetExtendedObject()
{
    ...
    // changed or has been freed.
        return interop::CallR(SMART_CLS(this)->fDereferencer, EXT_OBJECT(this), 0, nullptr);
    }
...
//- public methods -----------------------------------------------------------
cpyrt::CPPInstance* cpyrt::CPPInstance::Copy(void* cppinst, PyTypeObject* target)
{
...
{
    if (!IsSmart()) return interop::TCppScope_t{};
...
// method. The actual selection is in the __overload__() method of CPPOverload.
    ...
    if (!PyArg_ParseTuple(args, const_cast<char*>("O!O!:__dispatch__"),
            &cpyrt_PyText_Type, &mname, &cpyrt_PyText_Type, &sigarg))
        return nullptr;
...
static PyObject* op_get_smartptr(CPPInstance* self)
{
    ...
    return cpyrt::BindCppObjectNoCast(self->GetSmartObject(), SMART_TYPE(self), CPPInstance::kNoWrapConv);
}
...
static inline void* cast_actual(void* obj) {
    ...
    interop::TCppScope_t clActual = klass /* XXX: interop::GetActualClass(klass, address) */;
...
// of the C++ object that is held, as well as its type.
    ...
    if (self->IsSmart()) {
        ...
        repr = cpyrt_PyText_FromFormat(
            ...
            cpyrt_PyText_AsString(modname), clName.c_str(),
...
// Try to locate an std::hash for this type and use that if it exists
    ...
    interop::TCppScope_t stdhash = interop::GetFullScope("std::hash<"+interop::GetScopedFinalName(self->ObjectIsA())+">");
    if (stdhash) {
...
{
    static interop::TCppScope_t sOStringStreamID = interop::GetFullScope("std::ostringstream");
    std::ostringstream s;
    ...
    if (!ScopeFlagCheck(self, CPPScope::kNoOSInsertion)) {
        for (PyObject* pyname : {PyStrings::gLShift, PyStrings::gLShiftC, (PyObject*)0x01, (PyObject*)0x02}) {
            ...
            // normal lookup failed; attempt lazy install of global operator<<(ostream&, type&)
                ...
                interop::TCppScope_t rnsID = interop::GetScope(TypeManip::extract_namespace(rcname));
            ...
            // TODO: ToString() still leaks ...
                const std::string& pretty = interop::ToString(self->ObjectIsA(), self->GetObject());
                if (!pretty.empty())
...
// 2. Cling's pretty printing (not done through backend for performance reasons)
    if (!ScopeFlagCheck(self, CPPScope::kNoPrettyPrint)) {
        ...
        if (!printValue) {
            PyObject* gbl = PyDict_GetItemString(PySys_GetObject((char*)"modules"), "cppjit.gbl");
        ...
        // have to force the issue by working with a by-ptr object
            ...
            if (!(self->fFlags & CPPInstance::kIsReference)) {
                byref = BindCppObjectNoCast((interop::TCppObject_t)&cppobj,
        ...
        // explicit template lookup
            PyObject* clName = cpyrt_PyText_FromString(Utility::ClassName((PyObject*)self).c_str());
...
PyTypeObject CPPInstance_Type = {
    ...
    (char*)"cppjit.CPPInstance",    // tp_name
