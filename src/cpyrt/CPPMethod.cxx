...
cpyrt::PyCallArgs::~PyCallArgs() {
    ...
    if (fFlags & kDoItemDecref) {
        for (Py_ssize_t iarg = 0; iarg < cpyrt_PyArgs_GET_SIZE(fArgs, fNArgsf); ++iarg)
...
//----------------------------------------------------------------------------
inline PyObject* cpyrt::CPPMethod::ExecuteFast(
...
// code duplication with ProtectedCall()
    ...
    try {       // C++ try block
        result = fExecutor->Execute(fMethod, interop::TCppObject_t((void*)((intptr_t)self+offset)), ctxt);
    } catch (PyException&) {
    ...
    // attempt to set the exception to the actual type, to allow catching with the Python C++ type
        static interop::TCppScope_t exc_type = interop::GetFullScope("std::exception");
...
//----------------------------------------------------------------------------
inline PyObject* cpyrt::CPPMethod::ExecuteProtected(
    void* self, ptrdiff_t offset, CallContext* ctxt)
{
...
//----------------------------------------------------------------------------
bool cpyrt::CPPMethod::InitExecutor_(Executor*& executor, CallContext* /* ctxt */)
{
// install executor conform to the return type
    executor =
        (bool)fMethod == true ? CreateExecutor(interop::GetMethodReturnType(fMethod)) \
...
extern std::unordered_map<interop::TCppType_t, interop::TCppType_t> TypeReductionMap;

//- constructors and destructor ----------------------------------------------
cpyrt::CPPMethod::CPPMethod(
        interop::TCppScope_t scope, interop::TCppMethod_t method) :
    fMethod(method), fScope(scope), fExecutor(nullptr), fArgIndices(nullptr),
    fArgsRequired(-1)
{
   interop::TCppType_t result = interop::ResolveType(interop::GetMethodReturnType(fMethod));
...
PyObject* cpyrt::CPPMethod::GetPrototype(bool fa)
{
    ...
    return cpyrt_PyText_FromFormat("%s%s %s%s",
        (interop::IsStaticMethod(fMethod) ? "static " : ""),
...
PyObject* cpyrt::CPPMethod::GetTypeName()
{
    PyObject* cppname = cpyrt_PyText_FromString(
        (GetReturnTypeName() + \
        " (" + (fScope ? interop::GetScopedFinalName(fScope) + "::*)" : "*)")).c_str());
    cpyrt_PyText_AppendAndDel(&cppname, GetSignature(false /* show_formalargs */));
    return cppname;
}

//----------------------------------------------------------------------------
PyObject* cpyrt::CPPMethod::Reflex(interop::Reflex::RequestId_t request, interop::Reflex::FormatId_t format)
{
// C++ reflection tooling for methods.
    ...
    if (request == interop::Reflex::RETURN_TYPE) {
        ...
        if (format == interop::Reflex::OPTIMAL || format == interop::Reflex::AS_TYPE)
            scope = interop::GetScope(rtn);

        if (format == interop::Reflex::AS_STRING || (format == interop::Reflex::OPTIMAL && !scope))
            return cpyrt_PyText_FromString(rtn.c_str());
        else if (format == interop::Reflex::AS_TYPE || format == interop::Reflex::OPTIMAL) {
...
// add a small penalty to prefer non-const methods over const ones for get/setitem
    if (interop::IsConstMethod(fMethod) && interop::GetName(interop::TCppScope_t(fMethod.data)) == "operator[]")
        priority += -10;
...
// borrowed reference to cppjit.gbl module to use its dictionary to eval in
    static PyObject* gbl = PyDict_GetItemString(PySys_GetObject((char*)"modules"), "cppjit.gbl");
    ...
    if (!defvalue.empty()) {
        ...
        // the function lives, then in the global scope
            ...
            if (!interop::IsBuiltin(possible_scope)) {
                std::string cand_scope = interop::GetScopedFinalName(fScope)+"::"+possible_scope;
    ...
    // the error message to indicate where it's coming from)
        ...
        PyObject* pycode = Py_CompileString((char*)defvalue.c_str(), "cppjit_default_compiler", Py_eval_input);
        if (pycode) {
        ...
        if (!pyval && PyErr_Occurred() && silent) {
            PyErr_Clear();
            pyval = cpyrt_PyText_FromString(defvalue.c_str());    // allows continuation, but is likely to fail
        }
    ...
    PyErr_Format(PyExc_TypeError, "Could not construct default value for: %s", interop::GetMethodArgName(fMethod, iarg).c_str());
    return nullptr;
}


bool cpyrt::CPPMethod::IsConst() {
    return interop::IsConstMethod(GetMethod());
}
...
// Return the C++ pointer of this function
    return interop::GetFunctionAddress(fMethod, false /* don't check fast path envar */);
}
...
// minimum number of arguments when calling
    fArgsRequired = (int)((bool)fMethod == true ? interop::GetMethodReqArgs(fMethod) : 0);
...
bool cpyrt::CPPMethod::ProcessKwds(PyObject* self_in, PyCallArgs& cargs)
{
    ...
    Py_ssize_t nArgs = cpyrt_PyArgs_GET_SIZE(cargs.fArgs, cargs.fNArgsf) + (self_in ? 1 : 0);
...
// next, insert the keyword values
    ...
    for (Py_ssize_t ikey = 0; ikey < nKeys; ++ikey) {
        ...
        if (p == fArgIndices->end()) {
            SetPyError_(cpyrt_PyText_FromFormat("%s::%s got an unexpected keyword argument \'%s\'",
                interop::GetFinalName(fScope).c_str(), interop::GetName(interop::TCppScope_t(fMethod.data)).c_str(), ckey));
            return false;
        }
...
// fill out the positional arguments
    ...
    for (Py_ssize_t i = start; i < nArgs; ++i) {
        if (vArgs[i]) {
            SetPyError_(cpyrt_PyText_FromFormat("%s::%s got multiple values for argument %d",
                interop::GetFinalName(fScope).c_str(), interop::GetName(interop::TCppScope_t(fMethod.data)).c_str(), (int)i+1));
...
// no self, set error and lament
    SetPyError_(cpyrt_PyText_FromFormat(
        ...
        interop::GetFinalName(fScope).c_str(), interop::GetName(interop::TCppScope_t(fMethod.data)).c_str(),
        interop::GetFinalName(fScope).c_str()));
    return false;
}

//----------------------------------------------------------------------------
bool cpyrt::CPPMethod::ConvertAndSetArgs(cpyrt_PyArgs_t args, size_t nargsf, CallContext* ctxt)
{
...
// convert the arguments to the method call array
    ...
    for (int i = 0; i < (int)argc; ++i) {
        if (!fConverters[i]->SetArg(cpyrt_PyArgs_GET_ITEM(args, i), cppArgs[i], ctxt)) {
            SetPyError_(cpyrt_PyText_FromFormat("could not convert argument %d: %s", i+1, fConverters[i]->GetFailureMsg().c_str()));
            isOK = false;
            break;
        }
    }
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPMethod::Execute(void* self, ptrdiff_t offset, CallContext* ctxt)
{
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPMethod::Call(CPPInstance*& self,
    cpyrt_PyArgs_t args, size_t nargsf, PyObject* kwds, CallContext* ctxt)
{
// setup as necessary
...
// subobject. Using fScope here would yield a zero offset and corrupt memory.
    interop::TCppScope_t declaring = interop::GetParentScope(interop::TCppScope_t(fMethod.data));
    if (!declaring)
        declaring = fScope;

    ptrdiff_t offset = 0;
    if (derived && derived != declaring)
        offset = interop::GetBaseOffset(derived, declaring, object, 1 /* up-cast */);
...
PyObject *cpyrt::CPPMethod::GetSignatureTypes()
{
   ...
   PyDict_SetItem(signature_types_dict, cpyrt_PyText_FromString("return_type"), return_type_py);
   ...
   for (int iarg = 0; iarg < argcount; ++iarg) {
      const std::string &argtype_cpp = interop::GetMethodArgTypeAsString(fMethod, iarg);
      PyObject *argtype_py = cpyrt_PyText_FromString(argtype_cpp.c_str());
      PyTuple_SET_ITEM(parameter_types, iarg, argtype_py);
   }

   PyDict_SetItem(signature_types_dict, cpyrt_PyText_FromString("input_types"), parameter_types);
