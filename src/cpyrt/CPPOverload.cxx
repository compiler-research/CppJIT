...
public:
    ...
    PyObject* Call(CPPInstance*& self,
            cpyrt_PyArgs_t args, size_t nargsf, PyObject* kwds, CallContext* /* ctxt = 0 */) override {
                ...
                // else if the return value falls within the memory of 'this', force a lifeline
                    if (!(cppres->fFlags & CPPInstance::kIsValue)) {     // no need if the result is temporary
                        if (im_self->fFlags & CPPInstance::kIsValue)
                            ...
                            if (0 <= offset && offset < (ptrdiff_t)interop::SizeOf(im_self->ObjectIsA()))
                                 ll_action = 4;
                        }
                    }
                }
...
{
// Return this function's global dict (hard-wired to be the cppjit module); used
        ...
        // collect error message/trace (automatically clears exception, too)
            ...
            // this should not happen; set an error to prevent core dump and report
                ...
                PyErr_Format(PyExc_SystemError, "%s =>\n    %s",
                    cpyrt_PyText_AsString(sig), (char*)"nullptr result without error in overload call");
                Py_DECREF(sig);
            }
...
PyTypeObject CPPOverload_Type = {
    ...
    (char*)"cppjit.CPPOverload",        // tp_name
...
//- public members -----------------------------------------------------------
void cpyrt::CPPOverload::Set(const std::string& name, std::vector<PyCallable*>& methods)
{
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPOverload::FindOverload(const std::string& signature, int want_const)
{
...
PyObject* cpyrt::CPPOverload::FindOverload(PyObject *args_tuple, int want_const)
{
    Py_ssize_t n = PyTuple_Size(args_tuple);

    CPPOverload::Methods_t& methods = fMethodInfo->fMethods;

    // This value is set based on the maximum penalty in interop::CompareMethodArgType
