...
static PyObject* meta_getcppname(CPPScope* scope, void*)
{
    ...
    return cpyrt_PyText_FromString(interop::GetScopedFinalName(scope->fCppType).c_str());
}
...
static int meta_setmodule(CPPScope* scope, PyObject* value, void*)
{
    if ((void*)scope == (void*)&CPPInstance_Type) {
        PyErr_SetString(PyExc_AttributeError,
            "attribute \'__module__\' of 'cppjit.CPPScope\' objects is not writable");
        return -1;
    }
...
// printing of C++ classes
    ...
    PyObject* repr = cpyrt_PyText_FromFormat("<%s %s.%s at %p>",
        kind, cpyrt_PyText_AsString(modname), clName.c_str(), scope);
...
// providing the scope, which we don't want to do unnecessarily.
    ...
    if (CPPScope_CheckExact(subtype) && !interop::IsNamespace(((CPPScope*)subtype)->fCppType)) {
        if (interop::GetSmartPtrInfo(interop::GetScopedFinalName(((CPPScope*)subtype)->fCppType), &raw, &deref))
...
// initialization of class (based on metatype)
    ...
    // the consistent thing, thus allowing user control over naming
        result->fCppType = interop::GetScope(
            cpyrt_PyText_AsString(PyTuple_GET_ITEM(args, 0)));
    } else {
        ...
        if (3 <= PyTuple_GET_SIZE(args)) {
            ...
            if (0 < sz && !interop::IsNamespace(result->fCppType)) {
                ...
                // this is a cross-inheritance class)
                    PyObject* bname = cpyrt_PyText_FromString(interop::GetBaseName(result->fCppType, 0).c_str());
...
// maps for using namespaces and tracking objects
    if (!interop::IsNamespace(result->fCppType)) {
        static interop::TCppScope_t exc_type = interop::GetScope("exception", interop::GetScope("std"));
...
// more elaborate search in case of failure (eg. for inner classes on demand)
    ...
    // are available as "methods" even though they're not really that
        ...
        if (!attr) {
            ...
            if (interop::IsVariable(lookup_result) || interop::IsEnumConstant(lookup_result)) {
                attr = (PyObject*)CPPDataMember_New(scope, lookup_result);
            } else if (interop::IsTypedefed(lookup_result)) {
                interop::TCppType_t resolved_type = interop::ResolveType(interop::GetTypeFromScope(lookup_result));
                const std::string& cpd = TypeManip::compound(interop::GetTypeAsString(resolved_type));
                if (cpd == "*") {
                    interop::TCppScope_t tcl = interop::GetScopeFromType(interop::ResolveType(resolved_type));
                    if (tcl) {
    ...
    // enums types requested as type (rather than the constants)
        if (!attr) {
            interop::TCppScope_t enumerator = interop::GetUnderlyingScope(interop::GetNamed(name, scope));
    ...
    // refresh using list as necessary
        const std::vector<interop::TCppScope_t>& uv = interop::GetUsingNamespaces(klass->fCppType);
        ...
        // reload and reset weak refs
            for (auto uid : uv) {
                ...
                // the namespace may not otherwise be held, so tie the lifetimes
                    PyObject* llname = cpyrt_PyText_FromString(("__lifeline_"+uname).c_str());
...
// time for the class)
    ...
    // not found: prepare a full error report
        ...
        if (sklass) {
            topmsg = cpyrt_PyText_FromFormat("%s has no attribute \'%s\'. Full details:",
                cpyrt_PyText_AsString(sklass), cpyrt_PyText_AsString(pyname));
            Py_DECREF(sklass);
        } else {
            topmsg = cpyrt_PyText_FromFormat("no such attribute \'%s\'. Full details:",
...
// are needed to prevent unnecessary lookups and recursion.
    ...
    if (((CPPScope*)pyclass)->fFlags & CPPScope::kIsNamespace
        && !cpyrt::CPPDataMember_Check(pyval)
        && !cpyrt::CPPScope_Check(pyval)) {
        std::string name = cpyrt_PyText_AsString(pyname);
...
// Provide the requested reflection information.
    ...
    case interop::Reflex::IS_AGGREGATE:
      ...
      // for all public data types, too, and maybe for no padding?)
        if (interop::IsAggregate(klass->fCppType) || !interop::HasVirtualDestructor(klass->fCppType))
            Py_RETURN_TRUE;
        Py_RETURN_FALSE;
        break;
    }
