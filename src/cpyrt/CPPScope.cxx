...
add_template(PyObject* pyclass, const std::string& name,
  ...
  // specialization was requested, do not return the template yet
  if (ncl == name)
...
//= cpyrt type proxy construction/destruction =============================
...
static void meta_dealloc(CPPScope* scope) {
  if (scope->fFlags & CPPScope::kIsNamespace) {
    if (scope->fImp.fUsing) {
      for (auto pyobj : *scope->fImp.fUsing)
        Py_DECREF(pyobj);
      delete scope->fImp.fUsing;
      scope->fImp.fUsing = nullptr;
    }
  } else if (!(scope->fFlags & CPPScope::kIsPython)) {
    delete scope->fImp.fCppObjects;
    scope->fImp.fCppObjects = nullptr;
  }
...
static PyObject* meta_getmodule(CPPScope* scope, void*) {
  ...
  PyObject* pyscope = cpyrt::GetScopeProxy(parent_scope);
  if (pyscope) {
...
static int meta_setmodule(CPPScope* scope, PyObject* value, void*) {
  if ((void*)scope == (void*)&CPPInstance_Type) {
    PyErr_SetString(PyExc_AttributeError,
                    "attribute \'__module__\' of 'cppjit.CPPScope\' objects is "
                    "not writable");
    return -1;
  }
...
static PyObject* meta_repr(CPPScope* scope) {
  ...
  std::string clName = interop::GetFinalName(scope->fCppType);
  const char* kind =
...
static PyObject* pt_new(PyTypeObject* subtype, PyObject* args, PyObject* kwds) {
  ...
  if (!CPPScope_CheckExact(subtype) ||
      !strstr(subtype->tp_name, "_meta") /* convention */) {
    ...
    if (3 <= PyTuple_GET_SIZE(args)) {
      ...
      if (0 < sz && !interop::IsNamespace(result->fCppType)) {
        ...
        if (!InsertDispatcher(result, PyTuple_GET_ITEM(args, 1), dct, errmsg)) {
          PyErr_Format(PyExc_TypeError,
                       "no python-side overrides supported (%s)",
                       errmsg.str().c_str());
          return nullptr;
        } else {
  ...
  if (!interop::IsNamespace(result->fCppType)) {
    static interop::TCppScope_t exc_type =
        interop::GetScope("exception", interop::GetScope("std"));
    if (interop::IsSubclass(result->fCppType, exc_type))
      result->fFlags |= CPPScope::kIsException;
    if (!(result->fFlags & CPPScope::kIsPython))
      result->fImp.fCppObjects = new CppToPyMap_t;
    else {
      // special case: the C++ objects should be stored with the associated C++,
      // not Python, type
...
static PyObject* meta_getattro(PyObject* pyclass, PyObject* pyname) {
  ...
  if (attr) {
    if (CPPScope_Check(attr) && CPPScope_Check(pyclass) &&
        !(((CPPScope*)pyclass)->fFlags & CPPScope::kIsNamespace)) {
      // TODO: the goal here is to prevent typedefs that are shadowed in
      // subclasses to be found as from the base class. The better approach
      // would be to find all typedefs at class creation and insert placeholders
      // to flag here. The current typedef loop in gInterpreter won't do,
      // however.
      ...
      PyErr_Clear();
    } else if (CPPScope_Check(attr) && CPPScope_Check(pyclass) &&
               ((CPPScope*)attr)->fFlags & CPPScope::kIsException) {
  ...
  if (CPPScope_Check(attr) &&
      (((CPPScope*)attr)->fFlags & CPPScope::kIsException)) {
    // Instead of the CPPScope, return a fresh exception class derived from
    // CPPExcInstance.
    return CreateExcScopeProxy(attr, pyname, pyclass);
  }
  ...
  if (!attr) {
    Utility::FetchError(errors);
    interop::TCppScope_t scope = klass->fCppType;
    ...
    if (!attr) {
      ...
      } else if (interop::IsTypedefed(lookup_result)) {
        interop::TCppType_t resolved_type =
            interop::ResolveType(interop::GetTypeFromScope(lookup_result));
        const std::string& cpd =
            TypeManip::compound(interop::GetTypeAsString(resolved_type));
        if (cpd == "*") {
          interop::TCppScope_t tcl =
              interop::GetScopeFromType(interop::ResolveType(resolved_type));
          if (tcl) {
            typedefpointertoclassobject* tpc = PyObject_New(
                typedefpointertoclassobject, &TypedefPointerToClass_Type);
            tpc->fCppType = tcl;
            attr = (PyObject*)tpc;
          }
        }
    ...
    // function templates that have not been instantiated (namespaces _may_ have
    // already been taken care of, by their general function lookup above)
    if (!attr && !templated_functions_checked) {
      if (interop::ExistsMethodTemplate(scope, name))
        attr = add_template(pyclass, name);
      else {
        // for completeness in error reporting
        PyErr_Format(PyExc_TypeError, "\'%s\' is not a known C++ template",
                     name.c_str());
        Utility::FetchError(errors);
      }
    }
    ...
    if (!attr) {
      ...
      if (interop::IsEnumScope(enumerator)) {
        ...
        // for completeness in error reporting
        PyErr_Format(PyExc_TypeError, "\'%s\' is not a known C++ enum",
                     name.c_str());
        Utility::FetchError(errors);
      }
    }
  ...
  // if the attribute was not found but could possibly have been shadowed,
  // insert it into the dict now, to short-circuit future lookups (TODO: this is
  // part of the workaround described above of which the true solution is to
  // loop over all typedefs at creation time for the class)
...
static int meta_setattro(PyObject* pyclass, PyObject* pyname, PyObject* pyval) {
  // Global data and static data in namespaces is found lazily, thus if the
  // first use is setting of the global data by the user, it will not be
  // reflected on the C++ side, b/c there is no descriptor yet. This triggers
  // the creation for for such data as necessary. The many checks to narrow down
  // the specific case are needed to prevent unnecessary lookups and recursion.
...
static PyObject* meta_reflex(CPPScope* klass, PyObject* args) {
  ...
  switch (request) {
  case interop::Reflex::IS_NAMESPACE:
    if (klass->fFlags & CPPScope::kIsNamespace)
      Py_RETURN_TRUE;
    Py_RETURN_FALSE;
    break;
  case interop::Reflex::IS_AGGREGATE:
    // this is not the strict C++ definition of aggregates, but is closer to
    // what is needed for Numba and C calling conventions (TODO: probably have
    // to check for all public data types, too, and maybe for no padding?)
    if (interop::IsAggregate(klass->fCppType) ||
        !interop::HasVirtualDestructor(klass->fCppType))
      Py_RETURN_TRUE;
    Py_RETURN_FALSE;
    break;
  }
...
static PyObject* meta_dir(PyObject* self, PyObject*) {
  ...
  if (!cpyrt::CPPScope_Check((PyObject*)klass)) {
  ...
  interop::GetAllCppNames(klass->fCppType, cppnames);
  ...
  for (const std::string& name : cppnames) {
    if (name.find("__", 0, 2) != std::string::npos ||
        ...
        name.find("operator", 0, 8) != std::string::npos)
      continue;
...
static PyGetSetDef meta_getset[] = {
    ...
    {(char*)"__module__", (getter)meta_getmodule, (setter)meta_setmodule,
     nullptr, nullptr},
    {(char*)nullptr, nullptr, nullptr, nullptr, nullptr}};

//= cpyrt object proxy type type ==========================================
PyTypeObject CPPScope_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0)(char*) "cppjit.CPPScope", // tp_name
    sizeof(cpyrt::CPPScope),     // tp_basicsize
    0,                           // tp_itemsize
    0,                           // tp_dealloc
    ...
    (char*)"cpyrt metatype (internal)", // tp_doc
    0,                                  // tp_traverse
    0,                                  // tp_clear
    0,                                  // tp_richcompare
    0,                                  // tp_weaklistoffset
