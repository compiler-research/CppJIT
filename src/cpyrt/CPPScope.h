#ifndef CPYRT_CPPSCOPE_H
#define CPYRT_CPPSCOPE_H
...
/** Type object to hold class reference (this is only semantically a
   presentation of CPPScope instances, not in a C++ sense)
      @author  WLAV
      @date    07/06/2017
      @version 2.0
 */
...
template <typename T> inline bool CPPScope_Check(T* object) {
  // Short-circuit the type check by checking tp_new which all generated
  // subclasses of CPPScope inherit.
  return object && (Py_TYPE(object)->tp_new == CPPScope_Type.tp_new ||
                    PyObject_TypeCheck(object, &CPPScope_Type));
}

template <typename T> inline bool CPPScope_CheckExact(T* object) {
