...
// Python version is lower than 3.14, the version where that function got
// introduced.
...
struct cpyrt_tagPyCArgObject { // not public (but stable; note that older
...
// indices of ctypes types into the array caches (note that c_complex and
// c_fcomplex do not exist as types in ctypes)
#define ct_c_bool 0
#define ct_c_char 1
#define ct_c_shar 1
#define ct_c_wchar 2
  ...
  if (!cpt_t) {
    if (strcmp(gCTypesNames[nidx], "c_char") == 0) {
      ...
      if (ct_t) {
        ...
        cpt_t =
            (PyTypeObject*)PyObject_CallFunctionObjArgs(ptrcreat, ct_t, NULL);
        Py_DECREF(ptrcreat);
      }
    }
    if (cpt_t) {
...
static bool IsPyCArgObject(PyObject* pyobject) {
  ...
  if (!pycarg_type) {
    ...
    if (!ctmod)
      ...
      PyTypeObject* ct_t =
          (PyTypeObject*)PyObject_GetAttrString(ctmod, "c_int");
...
} _cppjit_ctypes_state;

} // unnamed namespace

static bool IsCTypesArrayOrPointer(PyObject* pyobject) {
  static _cppjit_ctypes_state* state = nullptr;
  if (!state) {
    PyObject* ctmod = PyImport_AddModule(
        "_ctypes"); // the extension module, not the Python one
    if (ctmod)
      state = (_cppjit_ctypes_state*)PyModule_GetState(ctmod);
  }

  // verify for object types that have a C payload
  if (state && (PyObject_IsInstance((PyObject*)Py_TYPE(pyobject),
                                    (PyObject*)state->PyCType_Type) ||
                PyObject_IsInstance((PyObject*)Py_TYPE(pyobject),
                                    (PyObject*)state->PyCPointerType_Type))) {
    return true;
  }
...
static inline bool SetLifeLine(PyObject* holder, PyObject* target,
                               intptr_t ref) {
  ...
  attr_name << "__" << ref;
  auto res =
      PyObject_SetAttrString(holder, (char*)attr_name.str().c_str(), target);
  return res != -1;
}
...
static bool HasLifeLine(PyObject* holder, intptr_t ref) {
  ...
  attr_name << "__" << ref;
  PyObject* res =
      PyObject_GetAttrString(holder, (char*)attr_name.str().c_str());
...
static inline cpyrt::CPPInstance*
GetCppInstance(PyObject* pyobject,
               interop::TCppScope_t klass = interop::TCppScope_t{},
  ...
  if (castobj) {
    if (CPPInstance_Check(castobj))
      ...
      if (pyclass) {
        CPPInstance* pytmp =
            (CPPInstance*)PyObject_Call(pyclass, castobj, NULL);
        Py_DECREF(pyclass);
...
static inline bool StrictBool(PyObject* pyobject, cpyrt::CallContext* ctxt) {
  using namespace cppjit::cpyrt;
  if (!AllowImplicit(ctxt) && !PyBool_Check(pyobject)) {
    if (!NoImplicit(ctxt))
      ctxt->fFlags |= CallContext::kHaveImplicit;
...
static inline bool cpyrt_PyLong_AsBool(PyObject* pyobject) {
...
// range-checking python integer to C++ integer conversion (prevents p2.7 silent
// conversions)
  ...
  static inline type cpyrt_PyLong_As##name(PyObject* pyobject) {               \
    if (!(PyLong_Check(pyobject) || PyInt_Check(pyobject))) {                  \
      ...
      PyErr_SetString(PyExc_TypeError,                                         \
                      #type " conversion expects an integer object");          \
      return (type) - 1;                                                       \
    }                                                                          \
    long l = PyLong_AsLong(pyobject);                                          \
    if (l < limit_low || limit_high < l) {                                     \
      PyErr_Format(PyExc_ValueError, "integer %ld out of range for " #type,    \
                   l);                                                         \
      return (type) - 1;                                                       \
    }                                                                          \
    return (type)l;                                                            \
...
static inline long cpyrt_PyLong_AsStrictLong(PyObject* pyobject) {
...
static inline PY_LONG_LONG cpyrt_PyLong_AsStrictLongLong(PyObject* pyobject) {
...
static inline bool CArraySetArg(PyObject* pyobject, cpyrt::Parameter& para,
  ...
  // general case of loading a C array pointer (void* + type code) as function
  // argument
  else if (pyobject == cpyrt::gNullPtrObject ||
           pyobject == cpyrt::gDefaultObject)
    para.fValue.fVoidp = nullptr;
  else {
    Py_ssize_t buflen = cpyrt::Utility::GetBuffer(pyobject, tc, size,
                                                  para.fValue.fVoidp, check);
    if (!buflen) {
      // stuck here as it's the least common
      if (cpyrt_PyLong_AsStrictInt(pyobject) == 0)
        para.fValue.fVoidp = nullptr;
      else {
...
static inline cpyrt::CPPInstance* ConvertImplicit(interop::TCppScope_t klass,
                                                  PyObject* pyobject,
                                                  cpyrt::Parameter& para,
                                                  cpyrt::CallContext* ctxt,
                                                  bool manage = true) {
  using namespace cppjit::cpyrt;

  // filter out copy and move constructors
  if (IsConstructor(ctxt->fFlags) && klass == ctxt->fCurScope &&
      ctxt->GetSize() == 1)
    return nullptr;
  ...
  if (!AllowImplicit(ctxt)) {
    ...
    if (!(pytype == &PyList_Type ||
          pytype == &PyTuple_Type)) { // || !CPPInstance_Check(pyobject))) {
      if (!NoImplicit(ctxt))
        ctxt->fFlags |= CallContext::kHaveImplicit;
      return nullptr;
    }
  ...
  // call constructor of argument type to attempt implicit conversion (disallow
  // any implicit conversions by the scope's constructor itself)
  PyObject* args = PyTuple_New(1);
  Py_INCREF(pyobject);
  PyTuple_SET_ITEM(args, 0, pyobject);
  ...
  if (!pytmp && PyTuple_CheckExact(pyobject)) {
    // special case: allow implicit conversion from given set of arguments in
    // tuple
    PyErr_Clear();
...
cppjit::cpyrt::Converter::~Converter() { /* empty */ }

//----------------------------------------------------------------------------
PyObject* cppjit::cpyrt::Converter::FromMemory(void*) {
...
//----------------------------------------------------------------------------
bool cppjit::cpyrt::Converter::ToMemory(PyObject*, void*,
                                        PyObject* /* ctxt */) {
  // could happen if no derived class override
...
//- helper macro's -----------------------------------------------------------
#define CPPJIT_IMPL_BASIC_CONVERTER_BODY(name, type, stype, ctype, F1, F2, tc) \
  ...
  if (val == (type) - 1 && PyErr_Occurred()) {                                 \
    ...
    if (Py_TYPE(pyobject) == ctypes_type) {                                    \
      PyErr_Clear();                                                           \
      val = *((type*)((cpyrt_tagCDataObject*)pyobject)->b_ptr);                \
...
#define CPPJIT_IMPL_BASIC_CONVERTER_METHODS(name, type, stype, ctype, F1, F2)  \
...
#define CPPJIT_IMPL_BASIC_CONVERTER_NI(name, type, stype, ctype, F1, F2, tc)   \
  bool cpyrt::name##Converter::SetArg(PyObject* pyobject, Parameter& para,     \
                                      CallContext* ctxt) {                     \
    if (!StrictBool(pyobject, ctxt))                                           \
      return false;                                                            \
    CPPJIT_IMPL_BASIC_CONVERTER_BODY(name, type, stype, ctype, F1, F2, tc)     \
  }                                                                            \
  CPPJIT_IMPL_BASIC_CONVERTER_METHODS(name, type, stype, ctype, F1, F2)

#define CPPJIT_IMPL_BASIC_CONVERTER_IB(name, type, stype, ctype, F1, F2, tc)   \
  bool cpyrt::name##Converter::SetArg(PyObject* pyobject, Parameter& para,     \
                                      CallContext* ctxt) {                     \
...
#define CPPJIT_IMPL_BASIC_CONVERTER_NB(name, type, stype, ctype, F1, F2, tc)   \
  bool cpyrt::name##Converter::SetArg(PyObject* pyobject, Parameter& para,     \
                                      CallContext* /*ctxt*/) {                 \
...
//----------------------------------------------------------------------------
static inline int ExtractChar(PyObject* pyobject, const char* tname, int low,
                              int high) {
  int lchar = -1;
  if (PyBytes_Check(pyobject)) {
    ...
    if (lchar == -1 && PyErr_Occurred())
      ...
      PyErr_Format(PyExc_ValueError,
                   "integer to character: value %d not in range [%d,%d]", lchar,
                   low, high);
      lchar = -1;
    }
  } else
...
//----------------------------------------------------------------------------
#define CPPJIT_IMPL_REFCONVERTER_FROM_MEMORY(name, ctype)                      \
  PyObject* cpyrt::name##RefConverter::FromMemory(void* ptr) {                 \
    ...
    ((cpyrt_tagCDataObject*)ref)->b_ptr = (char*)ptr;                          \
    ((cpyrt_tagCDataObject*)ref)->b_needsfree = 0;                             \
    return ref;                                                                \
  }

//----------------------------------------------------------------------------
#define CPPJIT_IMPL_BASIC_CONST_REFCONVERTER(name, type, ctype, F1)            \
  bool cpyrt::Const##name##RefConverter::SetArg(                               \
  ...
  CPPJIT_IMPL_REFCONVERTER_FROM_MEMORY(Const##name, ctype)

//----------------------------------------------------------------------------
#define CPPJIT_IMPL_BASIC_CONST_CHAR_REFCONVERTER(name, type, ctype, low,      \
                                                  high)                        \
  bool cpyrt::Const##name##RefConverter::SetArg(                               \
      PyObject* pyobject, Parameter& para, CallContext* /* ctxt */) {          \
...
//----------------------------------------------------------------------------
#define CPPJIT_IMPL_BASIC_CHAR_CONVERTER(name, type, low, high)                \
  bool cpyrt::name##Converter::SetArg(PyObject* pyobject, Parameter& para,     \
                                      CallContext* /* ctxt */) {               \
    /* convert <pyobject> to C++ <<type>>, set arg for call, allow int -> char \
     */                                                                        \
  ...
  bool cpyrt::name##Converter::ToMemory(PyObject* value, void* address,        \
    ...
    if (PyBytes_Check(value))                                                  \
      ...
      cstr = cpyrt_PyText_AsStringAndSize(value, &len);                        \
    if (cstr) {                                                                \
      if (len != 1) {                                                          \
        PyErr_Format(PyExc_TypeError,                                          \
                     #type " expected, got string of size %zd", len);          \
        return false;                                                          \
      }                                                                        \
      *((type*)address) = (type)cstr[0];                                       \
      ...
      if (!(low <= l && l <= high)) {                                          \
        PyErr_Format(PyExc_ValueError,                                         \
                     "integer to character: value %ld not in range [%d,%d]",   \
                     l, low, high);                                            \
        return false;                                                          \
      }                                                                        \
      *((type*)address) = (type)l;                                             \
...
CPPJIT_IMPL_BASIC_CONVERTER_IB(Long, long, long, c_long, PyLong_FromLong,
                               cpyrt_PyLong_AsStrictLong, 'l')

//----------------------------------------------------------------------------
bool cpyrt::LongRefConverter::SetArg(PyObject* pyobject, Parameter& para,
                                     CallContext* /* ctxt */) {
  // convert <pyobject> to C++ long&, set arg for call

  if (Py_TYPE(pyobject) == GetCTypesType(ct_c_long)) {
    para.fValue.fVoidp = (void*)((cpyrt_tagCDataObject*)pyobject)->b_ptr;
    para.fTypeCode = 'V';
    return true;
  }
...
//----------------------------------------------------------------------------
// clang-format off
CPPJIT_IMPL_BASIC_CONST_CHAR_REFCONVERTER(Char,  char,          c_char,  CHAR_MIN,  CHAR_MAX)
CPPJIT_IMPL_BASIC_CONST_CHAR_REFCONVERTER(UChar, unsigned char, c_uchar,        0, UCHAR_MAX)
...
CPPJIT_IMPL_BASIC_CONST_REFCONVERTER(UInt,   unsigned int,   c_uint,      PyLongOrInt_AsULong)
CPPJIT_IMPL_BASIC_CONST_REFCONVERTER(Long,   long,           c_long,      cpyrt_PyLong_AsStrictLong)
CPPJIT_IMPL_BASIC_CONST_REFCONVERTER(ULong,  unsigned long,  c_ulong,     PyLongOrInt_AsULong)
CPPJIT_IMPL_BASIC_CONST_REFCONVERTER(LLong,  PY_LONG_LONG,   c_longlong,  cpyrt_PyLong_AsStrictLongLong)
CPPJIT_IMPL_BASIC_CONST_REFCONVERTER(ULLong, PY_ULONG_LONG,  c_ulonglong, PyLongOrInt_AsULong64)
// clang-format on

//----------------------------------------------------------------------------
bool cpyrt::IntRefConverter::SetArg(PyObject* pyobject, Parameter& para,
                                    CallContext* /* ctxt */) {
  ...
  Py_ssize_t buflen =
      Utility::GetBuffer(pyobject, 'i', sizeof(int), para.fValue.fVoidp);
...
//----------------------------------------------------------------------------
#define CPPJIT_IMPL_REFCONVERTER(name, ctype, type, code)                      \
  bool cpyrt::name##RefConverter::SetArg(PyObject* pyobject, Parameter& para,  \
                                         CallContext* /* ctxt */) {            \
    /* convert a reference to int to Python through ctypes pointer object */   \
    ...
    if (!res) {                                                                \
      PyErr_SetString(PyExc_TypeError,                                         \
                      "use ctypes." #ctype " for pass-by-ref of " #type);      \
      return false;                                                            \
    }                                                                          \
    para.fTypeCode = 'V';                                                      \
    return res;                                                                \
  }                                                                            \
  CPPJIT_IMPL_REFCONVERTER_FROM_MEMORY(name, ctype)

// clang-format off
CPPJIT_IMPL_REFCONVERTER(Bool,    c_bool,       bool,               '?');
CPPJIT_IMPL_REFCONVERTER(Char,    c_char,       char,               'b');
CPPJIT_IMPL_REFCONVERTER(WChar,   c_wchar,      wchar_t,            'u');
CPPJIT_IMPL_REFCONVERTER(Char16,  c_uint16,     char16_t,           'H');
CPPJIT_IMPL_REFCONVERTER(Char32,  c_uint32,     char32_t,           'I');
CPPJIT_IMPL_REFCONVERTER(SChar,   c_byte,       signed char,        'b');
...
CPPJIT_IMPL_REFCONVERTER(UInt,    c_uint,       unsigned int,       'I');
CPPJIT_IMPL_REFCONVERTER_FROM_MEMORY(Long, c_long);
CPPJIT_IMPL_REFCONVERTER(ULong,   c_ulong,      unsigned long,      'L');
CPPJIT_IMPL_REFCONVERTER(LLong,   c_longlong,   long long,          'q');
CPPJIT_IMPL_REFCONVERTER(ULLong,  c_ulonglong,  unsigned long long, 'Q');
CPPJIT_IMPL_REFCONVERTER(Float,   c_float,      float,              'f');
CPPJIT_IMPL_REFCONVERTER_FROM_MEMORY(Double, c_double);
CPPJIT_IMPL_REFCONVERTER(LDouble, c_longdouble, PY_LONG_DOUBLE,     'g');
// clang-format on

//----------------------------------------------------------------------------
// convert <pyobject> to C++ bool, allow int/long -> bool, set arg for call
CPPJIT_IMPL_BASIC_CONVERTER_NI(Bool, bool, long, c_bool, PyBool_FromLong,
                               cpyrt_PyLong_AsBool, 'l')

//----------------------------------------------------------------------------
CPPJIT_IMPL_BASIC_CHAR_CONVERTER(Char, char, CHAR_MIN, CHAR_MAX)
CPPJIT_IMPL_BASIC_CHAR_CONVERTER(UChar, unsigned char, 0, UCHAR_MAX)
...
PyObject* cpyrt::WCharConverter::FromMemory(void* address) {
...
bool cpyrt::Char16Converter::SetArg(PyObject* pyobject, Parameter& para,
  ...
  char16_t val =
      *(char16_t*)(PyBytes_AS_STRING(bstr) + sizeof(char16_t) /*BOM*/);
  Py_DECREF(bstr);
  para.fValue.fLong = (long)val;
  para.fTypeCode = 'U';
  return true;
...
PyObject* cpyrt::Char16Converter::FromMemory(void* address) {
  return PyUnicode_DecodeUTF16((const char*)address, sizeof(char16_t), nullptr,
                               nullptr);
}
...
bool cpyrt::Char32Converter::SetArg(PyObject* pyobject, Parameter& para,
  ...
  char32_t val =
      *(char32_t*)(PyBytes_AS_STRING(bstr) + sizeof(char32_t) /*BOM*/);
  Py_DECREF(bstr);
  para.fValue.fLong = (long)val;
  para.fTypeCode = 'U';
  return true;
...
PyObject* cpyrt::Char32Converter::FromMemory(void* address) {
  return PyUnicode_DecodeUTF32((const char*)address, sizeof(char32_t), nullptr,
                               nullptr);
}
...
//----------------------------------------------------------------------------
bool cpyrt::ULongConverter::SetArg(PyObject* pyobject, Parameter& para,
                                   CallContext* ctxt) {
...
PyObject* cpyrt::ULongConverter::FromMemory(void* address) {
  // construct python object from C++ unsigned long read at <address>
  return PyLong_FromUnsignedLong(*((unsigned long*)address));
}

bool cpyrt::ULongConverter::ToMemory(PyObject* value, void* address,
                                     PyObject* /* ctxt */) {
...
//----------------------------------------------------------------------------
PyObject* cpyrt::UIntConverter::FromMemory(void* address) {
  // construct python object from C++ unsigned int read at <address>
  return PyLong_FromUnsignedLong(*((unsigned int*)address));
}

bool cpyrt::UIntConverter::ToMemory(PyObject* value, void* address,
                                    PyObject* /* ctxt */) {
...
CPPJIT_IMPL_BASIC_CONVERTER_NB(Float, float, double, c_float,
                               PyFloat_FromDouble, PyFloat_AsDouble, 'f')
CPPJIT_IMPL_BASIC_CONVERTER_NB(Double, double, double, c_double,
                               PyFloat_FromDouble, PyFloat_AsDouble, 'd')

CPPJIT_IMPL_BASIC_CONVERTER_NB(LDouble, PY_LONG_DOUBLE, PY_LONG_DOUBLE,
                               c_longdouble, PyFloat_FromDouble,
                               PyFloat_AsDouble, 'g')

cpyrt::ComplexDConverter::ComplexDConverter(bool keepControl)
    : InstanceConverter(interop::GetFullScope("std::complex<double>"),
                        keepControl) {}

// special case for std::complex<double>, maps it to/from Python's complex
bool cpyrt::ComplexDConverter::SetArg(PyObject* pyobject, Parameter& para,
                                      CallContext* ctxt) {
...
PyObject* cpyrt::ComplexDConverter::FromMemory(void* address) {
  std::complex<double>* dc = (std::complex<double>*)address;
  return PyComplex_FromDoubles(dc->real(), dc->imag());
}

bool cpyrt::ComplexDConverter::ToMemory(PyObject* value, void* address,
                                        PyObject* ctxt) {
...
//----------------------------------------------------------------------------
bool cpyrt::DoubleRefConverter::SetArg(PyObject* pyobject, Parameter& para,
                                       CallContext* /* ctxt */) {
  ...
  // alternate, pass pointer from buffer
  Py_ssize_t buflen =
      Utility::GetBuffer(pyobject, 'd', sizeof(double), para.fValue.fVoidp);
...
//----------------------------------------------------------------------------
// clang-format off
CPPJIT_IMPL_BASIC_CONST_REFCONVERTER(Float,   float,          c_float,      PyFloat_AsDouble)
CPPJIT_IMPL_BASIC_CONST_REFCONVERTER(Double,  double,         c_double,     PyFloat_AsDouble)
CPPJIT_IMPL_BASIC_CONST_REFCONVERTER(LDouble, PY_LONG_DOUBLE, c_longdouble, PyFloat_AsDouble)
// clang-format on

//----------------------------------------------------------------------------
bool cpyrt::VoidConverter::SetArg(PyObject*, Parameter&, CallContext*) {
  // can't happen (unless a type is mapped wrongly), but implemented for
  // completeness
...
//----------------------------------------------------------------------------
bool cpyrt::LLongConverter::SetArg(PyObject* pyobject, Parameter& para,
                                   CallContext* ctxt) {
  // convert <pyobject> to C++ long long, set arg for call
  if (!ImplicitBool(pyobject, ctxt))
    return false;

  para.fValue.fLLong = cpyrt_PyLong_AsStrictLongLong(pyobject);
  if (PyErr_Occurred())
    return false;
  para.fTypeCode = 'q';
  return true;
...
PyObject* cpyrt::LLongConverter::FromMemory(void* address) {
  // construct python object from C++ long long read at <address>
  return PyLong_FromLongLong(*(PY_LONG_LONG*)address);
}

bool cpyrt::LLongConverter::ToMemory(PyObject* value, void* address,
                                     PyObject* /* ctxt */) {
...
//----------------------------------------------------------------------------
bool cpyrt::ULLongConverter::SetArg(PyObject* pyobject, Parameter& para,
                                    CallContext* ctxt) {
...
PyObject* cpyrt::ULLongConverter::FromMemory(void* address) {
...
bool cpyrt::ULLongConverter::ToMemory(PyObject* value, void* address,
                                      PyObject* /* ctxt */) {
...
bool cpyrt::CStringConverter::SetArg(PyObject* pyobject, Parameter& para,
  ...
  Py_ssize_t len;
  const char* cstr = cpyrt_PyText_AsStringAndSize(pyobject, &len);
  if (!cstr) {
  ...
  if (fMaxSize != std::string::npos && fMaxSize < fBuffer.size())
    if (PyErr_WarnEx(PyExc_RuntimeWarning,
                     (char*)"string too long for char array (truncated)",
                     1) < 0)
      return false;
...
PyObject* cpyrt::CStringConverter::FromMemory(void* address) {
...
bool cpyrt::CStringConverter::ToMemory(PyObject* value, void* address,
                                       PyObject* ctxt) {
  // convert <value> to C++ const char*, write it at <address>
  Py_ssize_t len;
  const char* cstr = cpyrt_PyText_AsStringAndSize(value, &len);
  if (!cstr)
    return false;
  ...
  // if address is available, and it wasn't set by this converter, assume a
  // byte-wise copy; otherwise assume a pointer copy (this relies on the
  // converter to be used for properties, or for argument passing, but not both
  // at the same time; this is currently the case)
...
PyObject* cpyrt::WCStringConverter::FromMemory(void* address) {
  // construct python object from C++ wchar_t* read at <address>
  if (address && *(wchar_t**)address) {
    if (fMaxSize !=
        std::wstring::npos) // need to prevent reading beyond boundary
      return PyUnicode_FromWideChar(*(wchar_t**)address, (Py_ssize_t)fMaxSize);
    // with unknown size
    return PyUnicode_FromWideChar(*(wchar_t**)address,
                                  wcslen(*(wchar_t**)address));
  }
...
bool cpyrt::WCStringConverter::ToMemory(PyObject* value, void* address,
  ...
  if (fMaxSize != std::wstring::npos && fMaxSize < (std::wstring::size_type)len)
    if (PyErr_WarnEx(PyExc_RuntimeWarning,
                     (char*)"string too long for wchar_t array (truncated)",
                     1) < 0)
      return false;
...
//----------------------------------------------------------------------------
#define CPYRT_WIDESTRING_CONVERTER(name, type, encode, decode, snull)          \
  bool cpyrt::name##Converter::SetArg(PyObject* pyobject, Parameter& para,     \
                                      CallContext* /* ctxt */) {               \
    ...
    if (address && *(type**)address) {                                         \
      if (fMaxSize != std::wstring::npos)                                      \
        return decode(*(const char**)address,                                  \
                      (Py_ssize_t)fMaxSize * sizeof(type), nullptr, nullptr);  \
      return decode(*(const char**)address,                                    \
                    std::char_traits<type>::length(*(type**)address) *         \
                        sizeof(type),                                          \
                    nullptr, nullptr);                                         \
    }                                                                          \
                                                                               \
  ...
  bool cpyrt::name##Converter::ToMemory(PyObject* value, void* address,        \
    ...
    if (fMaxSize != std::wstring::npos && maxbytes < len) {                    \
      ...
      len = maxbytes;                                                          \
                                                                               ...
                                                                               \
    memcpy(*((void**)address), PyBytes_AS_STRING(bstr) + sizeof(type) /*BOM*/, \
           len);                                                               \
    Py_DECREF(bstr);                                                           \
    /* debatable, but probably more convenient in most cases to null-terminate \
     * if enough space */                                                      \
    if (len / sizeof(type) < fMaxSize)                                         \
      (*(type**)address)[len / sizeof(type)] = snull;                          \
    return true;                                                               \
  }

CPYRT_WIDESTRING_CONVERTER(CString16, char16_t, PyUnicode_AsUTF16String,
                           PyUnicode_DecodeUTF16, u'\0')
CPYRT_WIDESTRING_CONVERTER(CString32, char32_t, PyUnicode_AsUTF32String,
                           PyUnicode_DecodeUTF32, U'\0')

//----------------------------------------------------------------------------
bool cpyrt::NonConstCStringConverter::SetArg(PyObject* pyobject,
                                             Parameter& para,
                                             CallContext* ctxt) {
  // attempt base class first (i.e. passing a string), but if that fails, try a
  // buffer
...
//----------------------------------------------------------------------------
PyObject* cpyrt::NonConstCStringConverter::FromMemory(void* address) {
...
//----------------------------------------------------------------------------
bool cpyrt::VoidArrayConverter::GetAddressSpecialCase(PyObject* pyobject,
                                                      void*& address) {
...
bool cpyrt::VoidArrayConverter::SetArg(PyObject* pyobject, Parameter& para,
  ...
  // allow ctypes voidp (which if got as a buffer will return void**, not
  // void*); use isintance instead of an exact check, b/c c_void_p is the type
  // mapper for typedefs of void* (typically opaque handles)
  ...
  if (IsCTypesArrayOrPointer(pyobject)) {
    void** payload = (void**)((cpyrt_tagCDataObject*)pyobject)->b_ptr;
    if (payload) {
  ...
  Py_ssize_t buflen =
      Utility::GetBuffer(pyobject, '*', 1, para.fValue.fVoidp, false);
...
//----------------------------------------------------------------------------
PyObject* cpyrt::VoidArrayConverter::FromMemory(void* address) {
...
//----------------------------------------------------------------------------
bool cpyrt::VoidArrayConverter::ToMemory(PyObject* value, void* address,
                                         PyObject* /* ctxt */) {
  // just convert pointer if it is a C++ object
...
public:
  StdSpanConverter(std::string const& typeName, interop::TCppScope_t klass,
...
bool cpyrt::StdSpanConverter::SetArg(PyObject* pyobject, Parameter& para,
  ...
  if (PyObject_GetBuffer(pyobject, &fBufinfo, PyBUF_FORMAT) == 0) {
    if (!strchr(fBufinfo.format, typecode)) {
      PyErr_Format(PyExc_TypeError,
                   ...
                   "'%s', but got format '%s'",
                   typecode, fTypeName.c_str(),
                   fBufinfo.format ? fBufinfo.format : "<null>");
      PyBuffer_Release(&fBufinfo);
      return false;
    }
    buflen =
        Utility::GetBuffer(pyobject, typecode, 1, para.fValue.fVoidp, false);
  }
  ...
  if (para.fValue.fVoidp && buflen != 0) {
    ...
    fBuffer = std::span<std::size_t>{(std::size_t*)para.fValue.fVoidp,
                                     static_cast<std::size_t>(buflen)};
    fHasBuffer = true;
    para.fValue.fVoidp = &fBuffer;
    para.fTypeCode = 'V';
    return true;
...
#define CPPJIT_IMPL_ARRAY_CONVERTER(name, ctype, type, code, suffix)           \
  cpyrt::name##ArrayConverter::name##ArrayConverter(cdims_t dims)              \
    ...
    fIsFixed = dims ? fShape[0] != UNKNOWN_SIZE : false;                       \
                                                                               ...
                                                                               \
  bool cpyrt::name##ArrayConverter::SetArg(                                    \
      PyObject* pyobject, Parameter& para, CallContext* ctxt) {                \
    ...
    if (!convOk) {                                                             \
      ...
      if (Py_TYPE(pyobject) == ctypes_type) {                                  \
        ...
        cpyrt_tagPyCArgObject* carg = (cpyrt_tagPyCArgObject*)pyobject;        \
        if (carg->obj && Py_TYPE(carg->obj) == ctypes_type) {                  \
          para.fValue.fVoidp =                                                 \
              (void*)((cpyrt_tagCDataObject*)carg->obj)->b_ptr;                \
          para.fTypeCode = 'p';                                                \
          convOk = true;                                                       \
        }                                                                      \
      } else if (LowLevelView_Check(pyobject) &&                               \
    ...
    return convOk;                                                             \
                                                                               ...
                                                                               \
  PyObject* cpyrt::name##ArrayConverter::FromMemory(void* address) {           \
    if (!fIsFixed)                                                             \
      return CreateLowLevelView##suffix((type**)address, fShape);              \
    return CreateLowLevelView##suffix(*(type**)address, fShape);               \
  }                                                                            \
                                                                               \
  bool cpyrt::name##ArrayConverter::ToMemory(PyObject* value, void* address,   \
                                             PyObject* ctxt) {                 \
...
//----------------------------------------------------------------------------
// clang-format off
CPPJIT_IMPL_ARRAY_CONVERTER(Bool,     c_bool,       bool,                 '?', )
CPPJIT_IMPL_ARRAY_CONVERTER(SChar,    c_char,       signed char,          'b', )
...
CPPJIT_IMPL_ARRAY_CONVERTER(UInt,     c_uint,       unsigned int,         'I', )
CPPJIT_IMPL_ARRAY_CONVERTER(Long,     c_long,       long,                 'l', )
CPPJIT_IMPL_ARRAY_CONVERTER(ULong,    c_ulong,      unsigned long,        'L', )
CPPJIT_IMPL_ARRAY_CONVERTER(LLong,    c_longlong,   long long,            'q', )
CPPJIT_IMPL_ARRAY_CONVERTER(ULLong,   c_ulonglong,  unsigned long long,   'Q', )
CPPJIT_IMPL_ARRAY_CONVERTER(Float,    c_float,      float,                'f', )
CPPJIT_IMPL_ARRAY_CONVERTER(Double,   c_double,     double,               'd', )
CPPJIT_IMPL_ARRAY_CONVERTER(LDouble,  c_longdouble, long double,          'g', )
CPPJIT_IMPL_ARRAY_CONVERTER(ComplexF, c_fcomplex,   std::complex<float>,  'z', )
CPPJIT_IMPL_ARRAY_CONVERTER(ComplexD, c_complex,    std::complex<double>, 'Z', )
// clang-format on

//----------------------------------------------------------------------------
bool cpyrt::CStringArrayConverter::SetArg(PyObject* pyobject, Parameter& para,
                                          CallContext* ctxt) {
  if (Py_TYPE(pyobject) == GetCTypesPtrType(ct_c_char_p) ||
      (1 < fShape.ndim() &&
       PyObject_IsInstance(pyobject, (PyObject*)GetCTypesType(ct_c_pointer)))) {
    // 2nd predicate is ebatable: it's a catch-all for ctypes-styled
    // multi-dimensional objects, which at this point does not check further
    // dimensionality
    para.fValue.fVoidp = (void*)((cpyrt_tagCDataObject*)pyobject)->b_ptr;
    para.fTypeCode = 'V';
    return true;
    ...
    for (size_t i = 0; i < len; ++i) {
      ...
      if (item) {
        Py_ssize_t sz;
        const char* p = cpyrt_PyText_AsStringAndSize(item, &sz);
        Py_DECREF(item);

        if (p)
          fBuffer.push_back(p);
        else {
          PyErr_Format(PyExc_TypeError, "could not convert item %d to string",
                       (int)i);
          return false;
        }
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CStringArrayConverter::FromMemory(void* address) {
  if (fIsFixed)
...
bool cpyrt::NullptrConverter::SetArg(PyObject* pyobject, Parameter& para,
                                     CallContext* /* ctxt */) {
...
//----------------------------------------------------------------------------
template <typename T>
static inline bool cpyrt_PyUnicodeAsBytes2Buffer(PyObject* pyobject,
                                                 T& buffer) {
  ...
  if (pybytes) {
    ...
    if (cstr)
      buffer = T{cstr, (typename T::size_type)len};
    Py_DECREF(pybytes);
    return (bool)cstr;
  }
...
#define CPPJIT_IMPL_STRING_AS_PRIMITIVE_CONVERTER(name, type, F1, F2)          \
  cpyrt::name##Converter::name##Converter(bool keepControl)                    \
      : InstanceConverter(interop::GetFullScope(#type), keepControl) {}        \
                                                                               \
  bool cpyrt::name##Converter::SetArg(PyObject* pyobject, Parameter& para,     \
                                      CallContext* ctxt) {                     \
    if (cpyrt_PyUnicodeAsBytes2Buffer(pyobject, fBuffer)) {                    \
      para.fValue.fVoidp = &fBuffer;                                           \
      para.fTypeCode = 'V';                                                    \
      return true;                                                             \
    }                                                                          \
  ...
  PyObject* cpyrt::name##Converter::FromMemory(void* address) {                \
    ...
    auto* empty = new type();                                                  \
    return BindCppObjectNoCast(interop::TCppObject_t((void*)empty), fClass,    \
                               CPPInstance::kIsOwner);                         \
  }                                                                            \
                                                                               \
  bool cpyrt::name##Converter::ToMemory(PyObject* value, void* address,        \
                                        PyObject* ctxt) {                      \
    if (cpyrt_PyUnicodeAsBytes2Buffer(value, *((type*)address)))               \
      return true;                                                             \
    return InstanceConverter::ToMemory(value, address, ctxt);                  \
  }

CPPJIT_IMPL_STRING_AS_PRIMITIVE_CONVERTER(STLString, std::string, c_str, size)

cpyrt::STLWStringConverter::STLWStringConverter(bool keepControl)
    : InstanceConverter(interop::GetFullScope("std::wstring"), keepControl) {}
...
PyObject* cpyrt::STLWStringConverter::FromMemory(void* address) {
  if (address)
    return PyUnicode_FromWideChar(((std::wstring*)address)->c_str(),
                                  ((std::wstring*)address)->size());
  wchar_t w = L'\0';
  return PyUnicode_FromWideChar(&w, 0);
}
...
cpyrt::STLStringViewConverter::STLStringViewConverter(bool keepControl)
    : InstanceConverter(interop::GetFullScope("std::string_view"),
                        keepControl) {}
...
bool cpyrt::STLStringViewConverter::SetArg(PyObject* pyobject, Parameter& para,
  ...
  if (CPPInstance_Check(pyobject)) {
    static interop::TCppScope_t sStringID =
        interop::GetUnderlyingScope(interop::GetFullScope("std::string"));
...
PyObject* cpyrt::STLStringViewConverter::FromMemory(void* address) {
  if (address)
    return InstanceConverter::FromMemory(address);
...
bool cpyrt::STLStringViewConverter::ToMemory(PyObject* value, void* address,
                                             PyObject* ctxt) {
  // common case of simple object assignment
  if (InstanceConverter::ToMemory(value, address, ctxt))
    return true;

  // assignment of a Python string; buffering done Python-side b/c str is
  // immutable
  Py_ssize_t len;
  const char* cstr = cpyrt_PyText_AsStringAndSize(value, &len);
  if (cstr) {
...
bool cpyrt::STLStringMoveConverter::SetArg(PyObject* pyobject, Parameter& para,
                                           CallContext* ctxt) {
...
bool cpyrt::InstancePtrConverter<ISCONST>::SetArg(PyObject* pyobject,
                                                  Parameter& para,
                                                  CallContext* ctxt) {
  // convert <pyobject> to C++ instance*, set arg for call
  CPPInstance* pyobj =
      GetCppInstance(pyobject, ISCONST ? fClass : interop::TCppScope_t{});
  if (!pyobj) {
    if (GetAddressSpecialCase(pyobject, para.fValue.fVoidp)) {
    ...
    // not a cppjit object (TODO: handle SWIG etc.)
    return false;
  }

  // smart pointers should only extract the pointer if this is NOT an implicit
  // conversion to another smart pointer
  if (pyobj->IsSmart() && IsConstructor(ctxt->fFlags) &&
      interop::IsSmartPtr(ctxt->fCurScope))
    return false;
...
//----------------------------------------------------------------------------
template <bool ISCONST>
bool cpyrt::InstancePtrConverter<ISCONST>::ToMemory(PyObject* value,
                                                    void* address,
                                                    PyObject* /* ctxt */) {
  // convert <value> to C++ instance, write it at <address>
  CPPInstance* pyobj =
      GetCppInstance(value, ISCONST ? fClass : interop::TCppScope_t{});
  if (!pyobj) {
    void* ptr = nullptr;
    if (GetAddressSpecialCase(value, ptr)) {
...
//----------------------------------------------------------------------------
bool cpyrt::InstanceConverter::SetArg(PyObject* pyobject, Parameter& para,
                                      CallContext* ctxt) {
  // convert <pyobject> to C++ instance, set arg for call
  CPPInstance* pyobj = GetCppInstance(pyobject, fClass);
  if (pyobj) {
    auto oisa = pyobj->ObjectIsA();
    if (oisa && ((oisa == (interop::IsTypedefed(fClass)
                               ? interop::GetUnderlyingScope(fClass)
                               : fClass)) ||
                 interop::IsSubclass(oisa, fClass))) {
      // calculate offset between formal and actual arguments
      para.fValue.fVoidp = pyobj->GetObject();
...
//----------------------------------------------------------------------------
PyObject* cpyrt::InstanceConverter::FromMemory(void* address) {
  // This should not need a cast (ie. BindCppObjectNoCast), but performing the
  // cast here means callbacks receive down-casted object when passed by-ptr,
  // which is needed for object identity. The latter case is assumed to be more
  // common than conversion of (global) objects.
  return BindCppObject((interop::TCppObject_t)address, fClass);
}

//----------------------------------------------------------------------------
bool cpyrt::InstanceConverter::ToMemory(PyObject* value, void* address,
                                        PyObject* /* ctxt */) {
  // assign value to C++ instance living at <address> through assignment
  // operator
  PyObject* pyobj = BindCppObjectNoCast(address, fClass);
  PyObject* result =
      PyObject_CallMethodOneArg(pyobj, PyStrings::gAssign, value);
  Py_DECREF(pyobj);
...
//----------------------------------------------------------------------------
bool cpyrt::InstanceRefConverter::SetArg(PyObject* pyobject, Parameter& para,
                                         CallContext* ctxt) {
  // convert <pyobject> to C++ instance&, set arg for call
  CPPInstance* pyobj =
      GetCppInstance(pyobject, fIsConst ? fClass : interop::TCppScope_t{});
  if (pyobj) {
...
//----------------------------------------------------------------------------
PyObject* cpyrt::InstanceRefConverter::FromMemory(void* address) {
  return BindCppObjectNoCast((interop::TCppObject_t)address, fClass,
                             CPPInstance::kIsReference);
}

//----------------------------------------------------------------------------
bool cpyrt::InstanceMoveConverter::SetArg(PyObject* pyobject, Parameter& para,
                                          CallContext* ctxt) {
  // convert <pyobject> to C++ instance&&, set arg for call
  CPPInstance* pyobj =
      GetCppInstance(pyobject, fClass, true /* accept_rvalue */);
...
bool cpyrt::InstancePtrPtrConverter<ISREFERENCE>::SetArg(PyObject* pyobject,
  ...
  if (!pyobj) {
    if (!ISREFERENCE &&
        (pyobject == gNullPtrObject || pyobject == gDefaultObject)) {
      // allow nullptr as a special case
      para.fValue.fVoidp = nullptr;
      para.fTypeCode = 'p';
      return true;
    }
    return false; // not a cppjit object (TODO: handle SWIG etc.)
  }
...
template <bool ISREFERENCE>
PyObject*
cpyrt::InstancePtrPtrConverter<ISREFERENCE>::FromMemory(void* address) {
  // construct python object from C++ instance* read at <address>
  return BindCppObject(*(void**)address, fClass,
                       CPPInstance::kIsReference | CPPInstance::kIsPtrPtr);
}

//----------------------------------------------------------------------------
template <bool ISREFERENCE>
bool cpyrt::InstancePtrPtrConverter<ISREFERENCE>::ToMemory(
...
template class cpyrt::InstancePtrConverter<true>;
template class cpyrt::InstancePtrConverter<false>;
template class cpyrt::InstancePtrPtrConverter<true>;
template class cpyrt::InstancePtrPtrConverter<false>;
} // namespace cppjit::cpyrt

//----------------------------------------------------------------------------
bool cpyrt::InstanceArrayConverter::SetArg(PyObject* pyobject, Parameter& para,
                                           CallContext* /* txt */) {
  // convert <pyobject> to C++ instance**, set arg for call
  ...
  // treat the first instance of the tuple as the start of the array, and pass
  // it by pointer (TODO: store and check sizes)
  ...
  if (interop::IsSubclass(((CPPInstance*)first)->ObjectIsA(), fClass)) {
    // no memory policies supported; set pointer (may be null) and declare
    // success
...
//----------------------------------------------------------------------------
PyObject* cpyrt::InstanceArrayConverter::FromMemory(void* address) {
  // construct python tuple of instances from C++ array read at <address>
  return BindCppObjectArray(*(char**)address, fClass, fShape);
}

//----------------------------------------------------------------------------
bool cpyrt::InstanceArrayConverter::ToMemory(PyObject* /* value */,
                                             void* /* address */,
                                             PyObject* /* ctxt */) {
...
// Cling WORKAROUND -- classes for STL iterators are completely undefined in
// that they come in a bazillion different guises, so just do whatever
bool cpyrt::STLIteratorConverter::SetArg(PyObject* pyobject, Parameter& para,
                                         CallContext* /* ctxt */) {
  if (!CPPInstance_Check(pyobject))
    return false;
...
//----------------------------------------------------------------------------
bool cpyrt::VoidPtrRefConverter::SetArg(PyObject* pyobject, Parameter& para,
                                        CallContext* /* ctxt */) {
...
//----------------------------------------------------------------------------
cpyrt::VoidPtrPtrConverter::VoidPtrPtrConverter(cdims_t dims,
...
//----------------------------------------------------------------------------
bool cpyrt::VoidPtrPtrConverter::SetArg(PyObject* pyobject, Parameter& para,
                                        CallContext* /* ctxt */) {
  // convert <pyobject> to C++ void**, set arg for call
  CPPInstance* pyobj = GetCppInstance(pyobject);
  if (pyobj) {
    ...
    cpyrt_tagPyCArgObject* carg = (cpyrt_tagPyCArgObject*)pyobject;
    if (carg->obj) {
      para.fValue.fVoidp = (void*)((cpyrt_tagCDataObject*)carg->obj)->b_ptr;
      para.fTypeCode = 'p';
      return true;
    }
  }

  // buffer objects are allowed under "user knows best" (this includes the
  // buffer interface to ctypes.c_void_p, which results in a void**)
  Py_ssize_t buflen =
      Utility::GetBuffer(pyobject, '*', 1, para.fValue.fVoidp, false);
...
//----------------------------------------------------------------------------
PyObject* cpyrt::VoidPtrPtrConverter::FromMemory(void* address) {
  // read a void** from address; since this is unknown, uintptr_t is used (user
  // can cast)
...
//----------------------------------------------------------------------------
bool cpyrt::PyObjectConverter::SetArg(PyObject* pyobject, Parameter& para,
                                      CallContext* /* ctxt */) {
...
PyObject* cpyrt::PyObjectConverter::FromMemory(void* address) {
...
bool cpyrt::PyObjectConverter::ToMemory(PyObject* value, void* address,
                                        PyObject* /* ctxt */) {
...
// cache mapping signature/return type to python callable and corresponding
// wrapper
typedef std::string RetSigKey_t;
static std::unordered_map<RetSigKey_t, std::vector<void*>> sWrapperFree;
static std::unordered_map<RetSigKey_t, std::unordered_map<PyObject*, void*>>
    sWrapperLookup;
static std::unordered_map<PyObject*, std::pair<void*, RetSigKey_t>>
    sWrapperWeakRefs;
...
static void* PyFunction_AsCPointer(PyObject* pyobject,
  ...
  if (PyObject_IsInstance(pyobject, (PyObject*)GetCTypesType(ct_c_funcptr))) {
    ...
    void* fptr = *(void**)((cpyrt_tagCDataObject*)pyobject)->b_ptr;
    return fptr;
  }

  if (PyCallable_Check(pyobject) &&
      (allowCppInstance || !CPPInstance_Check(pyobject))) {
      ...
      if (freewrap != sWrapperFree.end() && !freewrap->second.empty()) {
        ...
        *(sWrapperReference[wpraddress]) = pyobject;
        ...
        if (wref)
          ...
          PyErr_Clear(); // happens for builtins which don't need this
      }
    }
    ...
    if (!wpraddress) {
      ...
      for (int i = 0; i < nArgs; ++i)
        code << ", pyargs[" << i << "]";
      code << ", NULL);\n"
              "    else PyErr_SetString(PyExc_TypeError, \"callable was "
              "deleted\");\n";
...
bool cpyrt::FunctionPointerConverter::SetArg(PyObject* pyobject,
                                             Parameter& para,
                                             CallContext* ctxt) {
...
PyObject* cpyrt::FunctionPointerConverter::FromMemory(void* address) {
  // A function pointer in clang is represented by a Type, not a FunctionDecl
...
bool cpyrt::FunctionPointerConverter::ToMemory(PyObject* pyobject,
                                               void* address, PyObject* ctxt) {
  // special case: allow nullptr singleton:
  if (pyobject == gNullPtrObject || pyobject == gDefaultObject) {
    *((void**)address) = nullptr;
    return true;
...
PyObject* cpyrt::StdFunctionConverter::FromMemory(void* address) {
  return fConverter->FromMemory(address);
}

bool cpyrt::StdFunctionConverter::ToMemory(PyObject* value, void* address,
                                           PyObject* ctxt) {
...
bool cpyrt::SmartPtrConverter::SetArg(PyObject* pyobject, Parameter& para,
                                      CallContext* ctxt) {
  ...
  if (interop::TCppScope_t tsmart = pyobj->GetSmartIsA()) {
  ...
  if (!pyobj->IsSmart() && interop::IsSubclass(oisa, fSmartPtrType)) {
    // calculate offset between formal and actual arguments
    para.fValue.fVoidp = pyobj->GetObject();
  ...
  if (!pyobj->IsSmart() && interop::IsSubclass(oisa, fUnderlyingType)) {
    ...
    // copy internals from the fresh smart object to the original, making it
    // smart
    pyobj->GetObjectRaw() = pysmart->GetSmartObject();
    pyobj->SetSmart(
        CreateScopeProxy(fSmartPtrType)); //(PyObject*)Py_TYPE(pysmart));
    pyobj->PythonOwns();
    pysmart->CppOwns();
    Py_DECREF(pysmart);
  ...
  // final option, try mapping pointer types held (TODO: do not allow for
  // non-const ref) Match on the declared underlying type, not the (possibly
  // downcast) dereferenced object: a unique_ptr<Base> holding a Derived is
  // still a unique_ptr<Base> and must not pass where a unique_ptr<Derived> is
  // expected.
  if (pyobj->IsSmart() &&
      interop::IsSubclass(pyobj->GetSmartUnderlyingType(), fUnderlyingType)) {
...
PyObject* cpyrt::SmartPtrConverter::FromMemory(void* address) {
...
bool cpyrt::SmartPtrConverter::ToMemory(PyObject* value, void* address,
                                        PyObject*) {
  // assign value to C++ instance living at <address> through assignment
  // operator (this is similar to InstanceConverter::ToMemory, but prevents
  // wrapping the smart ptr)
  PyObject* pyobj =
      BindCppObjectNoCast(address, fSmartPtrType, CPPInstance::kNoWrapConv);
  PyObject* result =
      PyObject_CallMethodOneArg(pyobj, PyStrings::gAssign, value);
  Py_DECREF(pyobj);
...
cpyrt::InitializerListConverter::~InitializerListConverter() {
  for (Converter* converter : fConverters) {
    if (converter && converter->HasState())
      delete converter;
  }
  if (fBuffer)
    Clear();
...
void cpyrt::InitializerListConverter::Clear() {
  if (fValueType) {
    ...
    for (size_t i = 0; (fake->_M_array + i * fValueSize) != fake->_Last; ++i) {
      ...
      interop::CallDestructor(fValueType, (interop::TCppObject_t)memloc);
    }
  }
...
bool cpyrt::InitializerListConverter::SetArg(PyObject* pyobject,
                                             Parameter& para,
                                             CallContext* ctxt) {
  ...
  // convert the given argument to an initializer list temporary; this is purely
  ...
  void* buf = nullptr;
  Py_ssize_t buflen =
      Utility::GetBuffer(pyobject, '*', (int)fValueSize, buf, true);
  ...
  if (buf && buflen) {
    ...
    // Remove any errors set by GetBuffer(); note that if the argument was an
    // array that failed to extract because of a type mismatch, the following
    // will perform a (rather inefficient) copy. No warning is issued b/c e.g.
    // numpy doesn't do so either.
    PyErr_Clear();

    // Can only construct empty lists, so use a fake initializer list. For that
    // we need to construct default objects. Fail early if that cannot work.
    if (fValueType && !interop::IsDefaultConstructable(fValueType)) {
      PyErr_SetString(
          PyExc_TypeError,
          "default constructor needed for initializer list of objects");
      return false;
    }
    ...
    fBuffer = (void*)fake;
    fake->_M_array =
        (faux_initlist::iterator)((char*)fake + sizeof(faux_initlist));
#if defined(_LIBCPP_INITIALIZER_LIST) || defined(__GNUC__)
    ...
    for (size_t i = 0; (fake->_M_array + i * fValueSize) != fake->_Last; ++i) {
      ...
      if (item) {
        ...
        if (!converter) {
          ...
          void* memloc = (char*)fake->_M_array + i * fValueSize;
          if (fValueType) {
            // we need to construct a default object for the constructor to
            // assign into; this is clunky, but the use of a copy constructor
            // isn't much better as the Python object need not be a C++ object
            memloc = (void*)interop::Construct(fValueType, memloc).data;
            // We checked above that we are able to construct default objects of
            // fValueType.
            assert(memloc && ("failed to default construct object for type " +
                              fValueTypeName)
                                 .c_str());
            entries += 1;
          }
        ...
        PyErr_Format(PyExc_TypeError, "failed to get item %d from sequence",
                     (int)i);
...
public:
  NotImplementedConverter(PyObject* errorType, std::string const& message)
      : fErrorType{errorType}, fMessage{message} {}
  bool SetArg(PyObject*, Parameter&, CallContext* = nullptr) override;
...
//----------------------------------------------------------------------------
bool cpyrt::NotImplementedConverter::SetArg(PyObject*, Parameter&,
                                            CallContext*) {
  // raise a NotImplemented exception to take a method out of overload
  // resolution
...
static inline cppjit::cpyrt::Converter*
selectInstanceCnv(interop::TCppScope_t klass, const std::string& cpd,
                  cpyrt::cdims_t dims, bool isConst, bool control) {
  using namespace cppjit::cpyrt;
  ...
  if (cpd == "**" || cpd == "*[]" || cpd == "&*")
    ...
    if (isConst)
      result = new InstancePtrConverter<true>(klass, control);
    else
      result = new InstancePtrConverter<false>(klass, control);
  } else if (cpd == "&")
...
//- factories ----------------------------------------------------------------
CPYRT_EXPORT
cppjit::cpyrt::Converter*
cppjit::cpyrt::CreateConverter(const std::string& fullType, cdims_t dims) {
  // The matching of the fulltype to a converter factory goes through up to five
  // levels:
  //   1) full, exact match
  ...
  //
  // If all fails, void is used, which will generate a run-time warning when
  // used.
  ...
  const std::string& resolvedType = interop::ResolveName(fullType);
  ...
  //-- nothing? ok, collect information about the type and possible
  ...
  if (cpd.compare(0, 3, "*[]") == 0) {
    ...
    if (h != gConvFactories.end()) {
      // upstream treats the pointer type as the array element type, but that
      // pointer is treated as a low-level view as well, unless it's a
      // void*/char* so adjust the dims
      if (realType != "void" && realType != "char") {
        dim_t newdim = dims.ndim() == UNKNOWN_SIZE ? 2 : dims.ndim() + 1;
        dims_t newdims = dims_t(newdim);
        // TODO: sometimes the array size is known and can thus be verified;
        // however, currently the meta layer does not provide this information
  ...
  } else if (!cpd.empty() && (std::string::size_type)std::count(
                                 cpd.begin(), cpd.end(), '*') == cpd.size()) {
    ...
    if (h != gConvFactories.end())
      return (h->second)((!dims && 1 < cpd.size()) ? dims_t(cpd.size()) : dims);
             ...
             (std::string::size_type)std::count(cpd.begin(), cpd.end(), '[') ==
                 cpd.size() / 2) {
  ...
  if (realType.compare(0, 21, "std::initializer_list") == 0) {
    ...
    std::string value_type =
        realType.substr(pos + 1, realType.size() - pos - 2);
    return new InitializerListConverter(interop::GetScope(realType),
                                        value_type);
  }
  ...
  if (pos == 0 /* std:: */ || pos == 6 /* const std:: */) {
    ...
    Converter* cnv = selectInstanceCnv(interop::GetScope(realType), cpd, dims,
                                       isConst, control);
  ...
  if (pos == 0 /* no std:: */ || pos == 5 /* with std:: */ ||
    ...
    interop::TCppScope_t span_scope = interop::GetScope(realType);
    interop::TCppType_t vtype = interop::ResolveType(
        interop::GetTypeFromScope(interop::GetNamed("value_type", span_scope)));
    if (span_scope && vtype) {
      std::string value_type = interop::GetTypeAsString(vtype);
  ...
  if (!result && cpd == "&&") {
    ...
    result = new NotImplementedConverter{PyExc_NotImplementedError,
  ...
  if (!result && h != gConvFactories.end())
    ...
    if (cpd.size() == 2 && cpd != "&&") // "**", "*[]", "*&"
      ...
      result = new VoidArrayConverter(/* keepControl= */ true,
                                      failure_msg); // "user knows best"
    else
      // fails on use
      result = new NotImplementedConverter{
          PyExc_NotImplementedError, "this method cannot (yet) be called"};
  }
...
CPYRT_EXPORT
cppjit::cpyrt::Converter*
cppjit::cpyrt::CreateConverter(interop::TCppType_t type, cdims_t dims) {
  // The matching of the fulltype to a converter factory goes through up to five
  // levels:
  //   1) full, exact match
  //   2) match of decorated, unqualified type
  ...
  // an exactly matching converter is best
  std::string fullType = interop::GetTypeAsString(type);
  ConvFactories_t::iterator h = gConvFactories.find(fullType);
  if (h != gConvFactories.end()) {
    return (h->second)(dims);
  }
  ...
  interop::TCppType_t realType =
      interop::ResolveType(interop::GetRealType(type));
  std::string realTypeStr = interop::GetTypeAsString(realType);
  std::string realUnresolvedTypeStr =
      TypeManip::clean_type(fullType, false, true);
  ...
  if (pos == 0 /* std:: */ || pos == 6 /* const std:: */) {
    ...
    // get actual converter for normal passing
    Converter* cnv = selectInstanceCnv(interop::GetScopeFromType(realType), cpd,
                                       dims, isConst, control);
    ...
    if (cnv) {
      ...
      if (pos1 != std::string::npos && pos2 != std::string::npos) {
        auto sz1 = pos1 - pos - 14;
        if (resolvedTypeStr[pos + 14 + sz1 - 1] == ' ')
          sz1 -= 1;
  ...
  if (pos == 0 /* no std:: */ || pos == 5 /* with std:: */ ||
    ...
    // std::span<T>::value_type == std::remove_cv_t<T>: look it up
    interop::TCppScope_t span_scope = interop::GetScopeFromType(realType);
    interop::TCppType_t vtype = interop::ResolveType(
        interop::GetTypeFromScope(interop::GetNamed("value_type", span_scope)));
    if (span_scope && vtype) {
      std::string value_type = interop::GetTypeAsString(vtype);
  ...
  const std::string failure_msg(
      "Failed to convert type: " + fullType + "; resolved: " + resolvedTypeStr +
      "; real: " + realTypeStr +
      "; realUnresolvedType: " + realUnresolvedTypeStr + "; cpd: " + cpd);
...
//----------------------------------------------------------------------------
CPYRT_EXPORT
void cppjit::cpyrt::DestroyConverter(Converter* p) {
  if (p && p->HasState())
...
//----------------------------------------------------------------------------
CPYRT_EXPORT
bool cpyrt::RegisterConverter(const std::string& name, cf_t fac) {
...
inline static std::string::size_type dims2stringsz(cdims_t d) {
...
//-- aliasing special case: C complex (is binary compatible with C++
// std::complex)
#ifndef _WIN32
...
public:
  InitConvFactories_t() {
    ...
    cpyrt::ConvFactories_t& gf = gConvFactories;

    // factories for built-ins
    gf["bool"] = (cf_t) + [](cdims_t) {
      static BoolConverter c{};
      return &c;
    };
    gf["const bool&"] = (cf_t) + [](cdims_t) {
      static ConstBoolRefConverter c{};
      return &c;
    };
    gf["bool&"] = (cf_t) + [](cdims_t) {
      static BoolRefConverter c{};
      return &c;
    };
    gf["char"] = (cf_t) + [](cdims_t) {
      static CharConverter c{};
      return &c;
    };
    gf["const char&"] = (cf_t) + [](cdims_t) {
      static ConstCharRefConverter c{};
      return &c;
    };
    gf["char&"] = (cf_t) + [](cdims_t) {
      static CharRefConverter c{};
      return &c;
    };
    gf["signed char&"] = (cf_t) + [](cdims_t) {
      static SCharRefConverter c{};
      return &c;
    };
    gf["unsigned char"] = (cf_t) + [](cdims_t) {
      static UCharConverter c{};
      return &c;
    };
    gf["const unsigned char&"] = (cf_t) + [](cdims_t) {
      static ConstUCharRefConverter c{};
      return &c;
    };
    gf["unsigned char&"] = (cf_t) + [](cdims_t) {
      static UCharRefConverter c{};
      return &c;
    };
    gf["SCharAsInt"] = (cf_t) + [](cdims_t) {
      static SCharAsIntConverter c{};
      return &c;
    };
    gf["UCharAsInt"] = (cf_t) + [](cdims_t) {
      static UCharAsIntConverter c{};
      return &c;
    };
    gf["wchar_t"] = (cf_t) + [](cdims_t) {
      static WCharConverter c{};
      return &c;
    };
    gf["char16_t"] = (cf_t) + [](cdims_t) {
      static Char16Converter c{};
      return &c;
    };
    gf["char32_t"] = (cf_t) + [](cdims_t) {
      static Char32Converter c{};
      return &c;
    };
    gf["wchar_t&"] = (cf_t) + [](cdims_t) {
      static WCharRefConverter c{};
      return &c;
    };
    gf["char16_t&"] = (cf_t) + [](cdims_t) {
      static Char16RefConverter c{};
      return &c;
    };
    gf["char32_t&"] = (cf_t) + [](cdims_t) {
      static Char32RefConverter c{};
      return &c;
    };
    gf["int8_t"] = (cf_t) + [](cdims_t) {
      static Int8Converter c{};
      return &c;
    };
    gf["const int8_t&"] = (cf_t) + [](cdims_t) {
      static ConstInt8RefConverter c{};
      return &c;
    };
    gf["int8_t&"] = (cf_t) + [](cdims_t) {
      static Int8RefConverter c{};
      return &c;
    };
    gf["uint8_t"] = (cf_t) + [](cdims_t) {
      static UInt8Converter c{};
      return &c;
    };
    gf["const uint8_t&"] = (cf_t) + [](cdims_t) {
      static ConstUInt8RefConverter c{};
      return &c;
    };
    gf["uint8_t&"] = (cf_t) + [](cdims_t) {
    ...
    gf["const short&"] = (cf_t) + [](cdims_t) {
      static ConstShortRefConverter c{};
      return &c;
    };
    gf["short&"] = (cf_t) + [](cdims_t) {
      static ShortRefConverter c{};
      return &c;
    };
    gf["unsigned short"] = (cf_t) + [](cdims_t) {
      static UShortConverter c{};
      return &c;
    };
    gf["const unsigned short&"] = (cf_t) + [](cdims_t) {
      static ConstUShortRefConverter c{};
      return &c;
    };
    gf["unsigned short&"] = (cf_t) + [](cdims_t) {
      static UShortRefConverter c{};
      return &c;
    };
    gf["int"] = (cf_t) + [](cdims_t) {
      static IntConverter c{};
      return &c;
    };
    gf["int&"] = (cf_t) + [](cdims_t) {
      static IntRefConverter c{};
      return &c;
    };
    gf["const int&"] = (cf_t) + [](cdims_t) {
      static ConstIntRefConverter c{};
      return &c;
    };
    gf["unsigned int"] = (cf_t) + [](cdims_t) {
      static UIntConverter c{};
      return &c;
    };
    gf["const unsigned int&"] = (cf_t) + [](cdims_t) {
      static ConstUIntRefConverter c{};
      return &c;
    };
    gf["unsigned int&"] = (cf_t) + [](cdims_t) {
      static UIntRefConverter c{};
      return &c;
    };
    gf["long"] = (cf_t) + [](cdims_t) {
      static LongConverter c{};
      return &c;
    };
    gf["long&"] = (cf_t) + [](cdims_t) {
      static LongRefConverter c{};
      return &c;
    };
    gf["const long&"] = (cf_t) + [](cdims_t) {
      static ConstLongRefConverter c{};
      return &c;
    };
    gf["unsigned long"] = (cf_t) + [](cdims_t) {
      static ULongConverter c{};
      return &c;
    };
    gf["const unsigned long&"] = (cf_t) + [](cdims_t) {
      static ConstULongRefConverter c{};
      return &c;
    };
    gf["unsigned long&"] = (cf_t) + [](cdims_t) {
      static ULongRefConverter c{};
      return &c;
    };
    gf["long long"] = (cf_t) + [](cdims_t) {
      static LLongConverter c{};
      return &c;
    };
    gf["const long long&"] = (cf_t) + [](cdims_t) {
      static ConstLLongRefConverter c{};
      return &c;
    };
    gf["long long&"] = (cf_t) + [](cdims_t) {
      static LLongRefConverter c{};
      return &c;
    };
    gf["unsigned long long"] = (cf_t) + [](cdims_t) {
      static ULLongConverter c{};
      return &c;
    };
    gf["const unsigned long long&"] = (cf_t) + [](cdims_t) {
      static ConstULLongRefConverter c{};
      return &c;
    };
    gf["unsigned long long&"] = (cf_t) + [](cdims_t) {
      static ULLongRefConverter c{};
      return &c;
    };

    gf["float"] = (cf_t) + [](cdims_t) {
      static FloatConverter c{};
      return &c;
    };
    gf["const float&"] = (cf_t) + [](cdims_t) {
      static ConstFloatRefConverter c{};
      return &c;
    };
    gf["float&"] = (cf_t) + [](cdims_t) {
      static FloatRefConverter c{};
      return &c;
    };
    gf["double"] = (cf_t) + [](cdims_t) {
      static DoubleConverter c{};
      return &c;
    };
    gf["double&"] = (cf_t) + [](cdims_t) {
      static DoubleRefConverter c{};
      return &c;
    };
    gf["const double&"] = (cf_t) + [](cdims_t) {
      static ConstDoubleRefConverter c{};
      return &c;
    };
    gf["long double"] = (cf_t) + [](cdims_t) {
      static LDoubleConverter c{};
      return &c;
    };
    gf["const long double&"] = (cf_t) + [](cdims_t) {
      static ConstLDoubleRefConverter c{};
      return &c;
    };
    gf["long double&"] = (cf_t) + [](cdims_t) {
      static LDoubleRefConverter c{};
      return &c;
    };
    gf["std::complex<double>"] =
        (cf_t) + [](cdims_t) { return new ComplexDConverter{}; };
    gf["const std::complex<double>&"] =
        (cf_t) + [](cdims_t) { return new ComplexDConverter{}; };
    gf["void"] = (cf_t) + [](cdims_t) {
      static VoidConverter c{};
      return &c;
    };

    // pointer/array factories
    gf["bool ptr"] =
        (cf_t) + [](cdims_t d) { return new BoolArrayConverter{d}; };
    gf["signed char ptr"] =
        (cf_t) + [](cdims_t d) { return new SCharArrayConverter{d}; };
    gf["signed char**"] = (cf_t) + [](cdims_t) {
      return new SCharArrayConverter{{UNKNOWN_SIZE, UNKNOWN_SIZE}};
    };
    gf["const unsigned char*"] =
        (cf_t) + [](cdims_t d) { return new UCharArrayConverter{d}; };
    gf["unsigned char ptr"] =
        (cf_t) + [](cdims_t d) { return new UCharArrayConverter{d}; };
    ...
    gf["unsigned short ptr"] =
        (cf_t) + [](cdims_t d) { return new UShortArrayConverter{d}; };
    gf["int ptr"] = (cf_t) + [](cdims_t d) { return new IntArrayConverter{d}; };
    gf["unsigned int ptr"] =
        (cf_t) + [](cdims_t d) { return new UIntArrayConverter{d}; };
    gf["long ptr"] =
        (cf_t) + [](cdims_t d) { return new LongArrayConverter{d}; };
    gf["unsigned long ptr"] =
        (cf_t) + [](cdims_t d) { return new ULongArrayConverter{d}; };
    gf["long long ptr"] =
        (cf_t) + [](cdims_t d) { return new LLongArrayConverter{d}; };
    gf["unsigned long long ptr"] =
        (cf_t) + [](cdims_t d) { return new ULLongArrayConverter{d}; };
    gf["float ptr"] =
        (cf_t) + [](cdims_t d) { return new FloatArrayConverter{d}; };
    gf["double ptr"] =
        (cf_t) + [](cdims_t d) { return new DoubleArrayConverter{d}; };
    gf["long double ptr"] =
        (cf_t) + [](cdims_t d) { return new LDoubleArrayConverter{d}; };
    gf["std::complex<float> ptr"] =
        (cf_t) + [](cdims_t d) { return new ComplexFArrayConverter{d}; };
    gf["std::complex<double> ptr"] =
        (cf_t) + [](cdims_t d) { return new ComplexDArrayConverter{d}; };
    gf["void*"] =
        (cf_t) + [](cdims_t d) { return new VoidArrayConverter{(bool)d}; };
    ...
    // We always need these converters when cppjit is based on an unpatched
    // ROOT, because the "long long" types are always converted to Long64_t
    // and ULong64_t already at the ROOT Meta level.
    // See
    // https://github.com/root-project/root/issues/15872#issuecomment-2174092763
    ...
    gf["nullptr_t"] = (cf_t) + [](cdims_t) {
      static NullptrConverter c{};
      return &c;
    };
    ...
    gf["const char[]"] =
        (cf_t) + [](cdims_t) { return new CStringConverter{}; };
    gf["char*"] = (cf_t) + [](cdims_t d) {
      return new NonConstCStringConverter{dims2stringsz(d)};
    };
    gf["char[]"] = (cf_t) + [](cdims_t d) {
      return new NonConstCStringArrayConverter{d, true};
    };
    gf["signed char*"] = gf["char*"];
    ...
    // resolves to _Nullable). The cppjit backend doesn't strip away such Clang
    ...
    // [2]
    // https://github.com/llvm/llvm-project/blob/2078da43e25a4623cab2d0d60decddf709aaea28/libcxx/include/string#L1061
    gf["wchar_t*_Nonnull"] =
        (cf_t) + [](cdims_t) { return new WCStringConverter{}; };
    gf["char16_t*"] = (cf_t) + [](cdims_t) { return new CString16Converter{}; };
    gf["char16_t[]"] = (cf_t) + [](cdims_t d) {
      return new CString16Converter{dims2stringsz(d)};
    };
    gf["char32_t*"] = (cf_t) + [](cdims_t) { return new CString32Converter{}; };
    gf["char32_t[]"] = (cf_t) + [](cdims_t d) {
      return new CString32Converter{dims2stringsz(d)};
    };
    // TODO: the following are handled incorrectly upstream (char16_t** where
    // char16_t* intended)?!
    gf["char16_t**"] = gf["char16_t*"];
    gf["char32_t**"] = gf["char32_t*"];
    gf["const char**"] = (cf_t) + [](cdims_t) {
      return new CStringArrayConverter{{UNKNOWN_SIZE, UNKNOWN_SIZE}, false};
    };
    gf["char**"] = gf["const char**"];
    gf["const char*[]"] =
        (cf_t) + [](cdims_t d) { return new CStringArrayConverter{d, false}; };
    gf["char*[]"] = (cf_t) + [](cdims_t d) {
      return new NonConstCStringArrayConverter{d, false};
    };
    gf["char ptr"] = gf["char*[]"];
    gf["std::basic_string<char>"] =
        (cf_t) + [](cdims_t) { return new STLStringConverter{}; };
    gf["const std::basic_string<char>&"] = gf["std::basic_string<char>"];
    gf["std::basic_string<char>&&"] =
        (cf_t) + [](cdims_t) { return new STLStringMoveConverter{}; };
    gf["const std::basic_string<char> &"] = gf["std::basic_string<char>"];
    gf["std::basic_string<char> &&"] =
        (cf_t) + [](cdims_t) { return new STLStringMoveConverter{}; };
    gf["std::basic_string_view<char>"] =
        (cf_t) + [](cdims_t) { return new STLStringViewConverter{}; };
    ...
    gf["const " STRINGVIEW " &"] = gf["std::basic_string_view<char>"];
    gf["std::wstring"] =
        (cf_t) + [](cdims_t) { return new STLWStringConverter{}; };
    ...
    gf["void ptr"] = gf["void**"];
    gf["PyObject*"] = (cf_t) + [](cdims_t) {
      static PyObjectConverter c{};
      return &c;
    };
