...
CPYRT_EXPORT Converter* CreateConverter(const std::string& fullType, cdims_t dims = 0);
CPYRT_EXPORT Converter* CreateConverter(cppjit::interop::TCppType_t type, cdims_t dims = 0);
...
CPYRT_EXPORT bool RegisterConverterAlias(const std::string& name, const std::string& target);
...
public:
    InstancePtrConverter(cppjit::interop::TCppScope_t klass, bool keepControl = false, const std::string &failureMsg = std::string()) :
        VoidArrayConverter(keepControl, failureMsg), fClass(cppjit::interop::GetUnderlyingScope(klass)) {}
