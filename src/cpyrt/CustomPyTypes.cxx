...
static PyObject* tptc_call(typedefpointertoclassobject* self, PyObject* args, PyObject* /* kwds */)
{
    ...
    if (!PyArg_ParseTuple(args, const_cast<char*>("|L"), &addr))
        return nullptr;
    return BindCppObjectNoCast(interop::TCppObject_t((void*)addr), self->fCppType);
}
...
PyTypeObject TypedefPointerToClass_Type = {
    ...
    (char*)"cppjit.TypedefPointerToClass",// tp_name
...
static PyObject* vectoriter_iternext(vectoriterobject* vi) {
    ...
    // (or at least not during the loop anyway). This gains 2x in performance.
        interop::TCppObject_t cppobj((void*)((ptrdiff_t)vi->vi_data + vi->vi_stride * vi->ii_pos));
        if (vi->vi_flags & vectoriterobject::kIsPolymorphic)
            result = cpyrt::BindCppObject(*(void**)cppobj.data, vi->vi_klass, cpyrt::CPPInstance::kNoMemReg);
        else
            result = cpyrt::BindCppObjectNoCast(cppobj, vi->vi_klass, cpyrt::CPPInstance::kNoMemReg);
...
PyTypeObject VectorIter_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0)
    (char*)"cppjit.vectoriter",    // tp_name
