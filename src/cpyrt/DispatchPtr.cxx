...
//-----------------------------------------------------------------------------
PyObject* cppjit::cpyrt::DispatchPtr::Get(bool borrowed) const
{
    ...
    if (fPyWeakRef) {
        PyObject* disp = cpyrt_GetWeakRef(fPyWeakRef);
...
//-----------------------------------------------------------------------------
cppjit::cpyrt::DispatchPtr::DispatchPtr(PyObject* pyobj, bool strong) : fPyHardRef(nullptr)
{
    PythonGILRAII python_gil_raii;
    if (strong) {
        Py_INCREF(pyobj);
...
//-----------------------------------------------------------------------------
cppjit::cpyrt::DispatchPtr::DispatchPtr(const DispatchPtr& other, void* cppinst) : fPyWeakRef(nullptr)
{
    PythonGILRAII python_gil_raii;
...
//-----------------------------------------------------------------------------
cppjit::cpyrt::DispatchPtr::~DispatchPtr() {
...
// continued access
    PythonGILRAII python_gil_raii;
    if (fPyWeakRef) {
        PyObject* pyobj = cpyrt_GetWeakRef(fPyWeakRef);
...
//-----------------------------------------------------------------------------
cppjit::cpyrt::DispatchPtr& cppjit::cpyrt::DispatchPtr::assign(const DispatchPtr& other, void* cppinst)
{
    PythonGILRAII python_gil_raii;
    if (this != &other) {
...
//-----------------------------------------------------------------------------
void cppjit::cpyrt::DispatchPtr::PythonOwns()
{
    PythonGILRAII python_gil_raii;
...
//-----------------------------------------------------------------------------
void cppjit::cpyrt::DispatchPtr::CppOwns()
{
    PythonGILRAII python_gil_raii;
// C++ maintains the hardref, keeping the PyObject alive w/o outstanding ref
    if (fPyWeakRef) {
        fPyHardRef = cpyrt_GetWeakRef(fPyWeakRef);
