...
//----------------------------------------------------------------------------
static inline void InjectMethod(interop::TCppMethod_t method, const std::string& mtCppName, std::ostringstream& code)
{
...
// build out the signature with predictable formal names
    ...
    code << ") ";
    if (interop::IsConstMethod(method)) code << "const ";
    code << "{\n";
    ...
    std::ostringstream& code, const CtorInfos_t& methods = CtorInfos_t{}, size_t idx = 0)
{
    ...
    // declare arguments
        ...
        for (Ctors_t::size_type i = 0; i < methods.size(); ++i) {
            ...
            if (i != 0) code << "__cppjit_internal::Sep*";
            size_t offset = (i != 0 ? arg_tots[i-1] : 0);
            for (size_t j = 0; j < nArgs; ++j) {
                if (i != 0 || j != 0) code << ", ";
                code << interop::GetMethodArgCanonTypeAsString(cinfo.first, j) << " a" << (j+offset);
            }
        }
        code << ") : ";
    ...
    // pass arguments to base constructors
        for (BaseInfos_t::size_type i = 0; i < base_infos.size(); ++i) {
            ...
            for (size_t j = first; j < arg_tots[i]; ++j) {
                if (j != first) code << ", ";
                bool isRValue = cpyrt::TypeManip::compound(\
                    interop::GetMethodArgCanonTypeAsString(methods[i].first, j-first)) == "&&";
...
bool cpyrt::InsertDispatcher(CPPScope* klass, PyObject* bases, PyObject* dct, std::ostringstream& err)
{
...
// collect all bases, error checking the hierarchy along the way
    ...
    for (Py_ssize_t ibase = 0; ibase < nBases; ++ibase) {
        ...
        interop::TCppScope_t basetype = ((CPPScope*)PyTuple_GET_ITEM(bases, ibase))->fCppType;
        ...
        base_infos.emplace_back(
            basetype, TypeManip::template_base(interop::GetFinalName(basetype)), interop::GetScopedFinalName(basetype));
    }
    ...
    for (BaseInfos_t::size_type ibase = 0; ibase < base_infos.size(); ++ibase) {
        ...
        for (auto &method : methods) {
            if (interop::IsConstructor(method)) {
                any_ctor_found = true;
                if ((interop::IsPublicMethod(method) || interop::IsProtectedMethod(method)) &&
                    ...
                    if (nreq == 0) default_found = true;
                        ...
                        const std::string& argtype = interop::GetMethodArgCanonTypeAsString(method, 0);
            ...
            // does not work here b/c there may be private overloads)
                if (interop::IsProtectedMethod(method)) {
                    ...
                    code << "  " << interop::GetMethodReturnTypeAsString(method) << " " << mtCppName << "(";
                    interop::TCppIndex_t nArgs = interop::GetMethodNumArgs(method);
                    for (interop::TCppIndex_t i = 0; i < nArgs; ++i) {
                        if (i != 0) code << ", ";
                        code << interop::GetMethodArgCanonTypeAsString(method, i) << " arg" << i;
                    }
                    code << ") ";
                    if (interop::IsConstMethod(method)) code << "const ";
                    ...
                    for (interop::TCppIndex_t i = 0; i < nArgs; ++i) {
                        if (i != 0) code << ", ";
                        if (interop::IsRValueReferenceType(interop::GetMethodArgType(method, i)))
                            code << "std::move(arg" << i << ")";
                        else
                            code << "arg" << i;
                    }
...
// CPPConstructor to call the dispatcher's one instead of the base)
    interop::TCppScope_t disp = interop::GetFullScope("__cppjit_internal::"+derivedName);
    if (!disp) {
