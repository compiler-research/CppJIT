...
CPYRT_EXPORT Executor* CreateExecutor(cppjit::interop::TCppType_t type, cdims_t = 0);
...
CPYRT_EXPORT bool RegisterExecutorAlias(const std::string& name, const std::string& target);
CPYRT_EXPORT bool UnregisterExecutor(const std::string& name);

// helper for the actual call
CPYRT_EXPORT void* CallVoidP(cppjit::interop::TCppMethod_t, cppjit::interop::TCppObject_t, CallContext*);
