...
static cpyrt::LowLevelView* ll_new(PyTypeObject* subtype, PyObject*, PyObject*)
{
// Create a new low level ptr type
    cpyrt::LowLevelView* pyobj = (cpyrt::LowLevelView*)subtype->tp_alloc(subtype, 0);
    if (!pyobj)
        return nullptr;
...
{                                                                            \
    return PyBool_FromLong((long)((intptr_t)pyobj->fBufInfo.internal & flag));\
                                                                             ...
                                                                             \
static int ll_set##name(cpyrt::LowLevelView* pyobj, PyObject* value, void*)\
{                                                                            \
...
//---------------------------------------------------------------------------
static inline void* ptr_from_index(cpyrt::LowLevelView* llview, Py_ssize_t index)
{
...
static void* ptr_from_tuple(cpyrt::LowLevelView* llview, PyObject* tup)
{
    ...
    for (Py_ssize_t dim = 0; dim < nindices; dim++) {
        ...
        if (!((intptr_t)view.internal & cpyrt::LowLevelView::kIsFixed) && dim != view.ndim-1)
            ptr = *(char**)ptr;
    }
    return ptr;
}
...
//---------------------------------------------------------------------------
static int ll_ass_sub(cpyrt::LowLevelView* self, PyObject* key, PyObject* value)
{
    Py_buffer& view = self->fBufInfo;
    Py_buffer src;
...
// verify size match
    ...
    for (Py_ssize_t idim = 0; idim < view.ndim; ++idim) {
        ...
        if (nlen == cpyrt::UNKNOWN_SIZE || nlen == INT_MAX/view.itemsize /* fake 'max' */) {
    ...
    if (0 < oldsz) {
        ...
        if (oldsz != newsz) {
            ...
            PyErr_Format(PyExc_ValueError,
                "cannot reshape array of size %ld into shape %s", (long)oldsz, cpyrt_PyText_AsString(tas));
            Py_DECREF(tas);
            return nullptr;
        }
    }
...
//---------------------------------------------------------------------------
static PyObject* ll_array(cpyrt::LowLevelView* self, PyObject* args, PyObject* kwds)
{
...
// Expect as most a dtype from the arguments;
    ...
    // expect possible dtype from the arguments, otherwise take it from the type code
        ...
        PyObject* npfrombuf = PyObject_GetAttr(npmod, cpyrt::PyStrings::gFromBuffer);
...
//---------------------------------------------------------------------------
template<typename T>
static inline cpyrt::LowLevelView* CreateLowLevelViewT(
    T* address, cpyrt::cdims_t shape, const char* format = nullptr, const char* name = nullptr, Py_ssize_t itemsize = -1)
{
    using namespace cppjit::cpyrt;
    ...
    if (isfix) {
        ...
        if (isfix) (intptr_t&)view.internal |= cpyrt::LowLevelView::kIsFixed;
    }
...
template<typename T>
static inline cpyrt::LowLevelView* CreateLowLevelViewT(
    T** address, cpyrt::cdims_t shape, const char* format = nullptr, const char* name = nullptr)
{
    using namespace cppjit::cpyrt;
