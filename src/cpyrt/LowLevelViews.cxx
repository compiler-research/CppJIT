...
// memory, not a Python object with a buffer interface), it uses the cpyrt
...
static cpyrt::LowLevelView* ll_new(PyTypeObject* subtype, PyObject*,
  ...
  cpyrt::LowLevelView* pyobj =
      (cpyrt::LowLevelView*)subtype->tp_alloc(subtype, 0);
  if (!pyobj)
    return nullptr;

  memset(&pyobj->fBufInfo, 0, sizeof(Py_buffer));
  (intptr_t&)pyobj->fBufInfo.internal |= cpyrt::LowLevelView::kIsCppArray;
  pyobj->fBuf = nullptr;
...
//----------------------------------------------------------------------------
static void ll_dealloc(cpyrt::LowLevelView* pyobj) {
  ...
  if ((intptr_t)pyobj->fBufInfo.internal & cpyrt::LowLevelView::kIsOwner) {
    if ((intptr_t)pyobj->fBufInfo.internal & cpyrt::LowLevelView::kIsCppArray)
      delete[] pyobj->fBuf;
    else
      free(pyobj->fBuf);
  }

  if (pyobj->fElemCnv != pyobj->fConverter && pyobj->fElemCnv &&
...
// clang-format off
CPYRT_LL_FLAG_GETSET(ownership, cpyrt::LowLevelView::kIsOwner,    __python_owns__)
CPYRT_LL_FLAG_GETSET(cpparray,  cpyrt::LowLevelView::kIsCppArray, __cpp_array__)
// clang-format on
...
//---------------------------------------------------------------------------
static inline int last_dim_is_contiguous(const Py_buffer* dest,
                                         const Py_buffer* src) {
...
static bool equiv_structure(const Py_buffer* dest, const Py_buffer* src) {
  ...
  // is identical.
  if (strcmp(dest->format, src->format) != 0 ||
      dest->itemsize != src->itemsize || !equiv_shape(dest, src)) {
    PyErr_SetString(PyExc_ValueError, "low level pointer assignment: lvalue "
...
//---------------------------------------------------------------------------
static void copy_base(const Py_ssize_t* shape, Py_ssize_t itemsize, char* dptr,
                      const Py_ssize_t* dstrides, const Py_ssize_t* dsuboffsets,
                      char* sptr, const Py_ssize_t* sstrides,
                      const Py_ssize_t* ssuboffsets, char* mem) {
...
static int copy_single(Py_buffer* dest, Py_buffer* src) {
  ...
  copy_base(dest->shape, dest->itemsize, (char*)dest->buf, dest->strides,
            dest->suboffsets, (char*)src->buf, src->strides, src->suboffsets,
            mem);
...
static char* lookup_dimension(Py_buffer& view, char* ptr, int dim,
                              Py_ssize_t index) {
  ...
  if (index < 0) {
    if (nitems != cpyrt::UNKNOWN_SIZE)
      index += nitems;
    else {
      PyErr_Format(
          PyExc_IndexError,
          "negative index not supported on dimension %d with unknown size",
          dim + 1);
      return nullptr;
    }
  }

  if (view.strides[dim] == cpyrt::UNKNOWN_SIZE) {
    PyErr_Format(
        PyExc_IndexError,
        "multi index not supported on dimension %d with unknown stride",
        dim + 1);
    return nullptr;
  }

  if (nitems != cpyrt::UNKNOWN_SIZE && (index < 0 || index >= nitems)) {
    PyErr_Format(PyExc_IndexError, "index out of bounds on dimension %d",
                 dim + 1);
    return nullptr;
  }
...
//---------------------------------------------------------------------------
static inline void* ptr_from_index(cpyrt::LowLevelView* llview,
                                   Py_ssize_t index) {
...
//---------------------------------------------------------------------------
static void* ptr_from_tuple(cpyrt::LowLevelView* llview, PyObject* tup) {
  Py_buffer& view = llview->fBufInfo;
  ...
  for (Py_ssize_t dim = 0; dim < nindices; dim++) {
    ...
    if (!((intptr_t)view.internal & cpyrt::LowLevelView::kIsFixed) &&
        dim != view.ndim - 1)
      ptr = *(char**)ptr;
  }
  return ptr;
...
//= mapping methods =========================================================
static Py_ssize_t ll_length(cpyrt::LowLevelView* self) {
  if (!self->get_buf())
    return 0;
...
static inline int init_slice(Py_buffer* base, PyObject* _key, int dim) {
  ...
  if (PySlice_GetIndicesEx(key, base->shape[dim], &start, &stop, &step,
                           &slicelength) < 0)
    return -1;
...
//---------------------------------------------------------------------------
static PyObject* ll_item(cpyrt::LowLevelView* self, Py_ssize_t index) {
...
//---------------------------------------------------------------------------
static PyObject* ll_item_multi(cpyrt::LowLevelView* self, PyObject* tup) {
  Py_buffer& view = self->fBufInfo;
  Py_ssize_t nindices = PyTuple_GET_SIZE(tup);
...
//---------------------------------------------------------------------------
static PyObject* ll_subscript(cpyrt::LowLevelView* self, PyObject* key) {
  Py_buffer& view = self->fBufInfo;
  ...
  if (PyIndex_Check(key)) {
    ...
    if (view.ndim == 1) {
      ...
      cpyrt::LowLevelView* ll = self->fCreator(slice_buf, {1, slicelen});
      if (!ll)
        delete[] slice_buf;
      else
        (intptr_t&)ll->fBufInfo.internal |= cpyrt::LowLevelView::kIsOwner;
...
//---------------------------------------------------------------------------
static int ll_ass_sub(cpyrt::LowLevelView* self, PyObject* key,
                      PyObject* value) {
  Py_buffer& view = self->fBufInfo;
  Py_buffer src;
...
//---------------------------------------------------------------------------
static int ll_getbuf(cpyrt::LowLevelView* self, Py_buffer* view, int flags) {
...
template <> struct typecode_traits<signed char> {
  static constexpr const char* format = "b";
  static constexpr const char* name = "SCharAsInt";
};
template <> struct typecode_traits<unsigned char> {
  static constexpr const char* format = "B";
  static constexpr const char* name = "UCharAsInt";
};
...
template <> struct typecode_traits<std::complex<float>> {
  static constexpr const char* format = "Zf";
  static constexpr const char* name = "std::complex<float>";
};
template <> struct typecode_traits<std::complex<double>> {
  static constexpr const char* format = "Zd";
  static constexpr const char* name = "std::complex<double>";
};
template <> struct typecode_traits<std::complex<int>> {
  static constexpr const char* format = "Zi";
  static constexpr const char* name = "std::complex<int>";
};
template <> struct typecode_traits<std::complex<long>> {
  static constexpr const char* format = "Zl";
  static constexpr const char* name = "std::complex<long>";
};

} // unnamed namespace

//---------------------------------------------------------------------------
bool cpyrt::LowLevelView::resize(size_t sz) {
...
//---------------------------------------------------------------------------
template <typename T>
static inline cpyrt::LowLevelView*
CreateLowLevelViewT(T* address, cpyrt::cdims_t shape,
  ...
  (intptr_t&)view.internal = cpyrt::LowLevelView::kIsCppArray; // assumed
  bool isfix = shape.ndim() != UNKNOWN_SIZE;
  if (isfix) {
    for (int i = 0; i < shape.ndim(); ++i)
      isfix = isfix && (shape[i] != UNKNOWN_SIZE);
    if (isfix)
      (intptr_t&)view.internal |= cpyrt::LowLevelView::kIsFixed;
  }
...
CreateLowLevelViewT(T** address, cpyrt::cdims_t shape,
  ...
  using namespace cppjit::cpyrt;
  LowLevelView* llp =
      (LowLevelView*)CreateLowLevelViewT((T*)address, shape, format, name);
  llp->set_buf((void**)address);
  return llp;
}

//---------------------------------------------------------------------------
#define CPPJIT_RET_W_CREATOR(type, fname)                                      \
  PyObject* (*c)(type, cdims_t) = &fname;                                      \
  ll->fCreator = (LowLevelView::Creator_t)c;                                   \
  return (PyObject*)ll

#define CPPJIT_IMPL_VIEW_CREATOR(type)                                         \
...
CPPJIT_IMPL_VIEW_CREATOR(bool);
CPPJIT_IMPL_VIEW_CREATOR(signed char);
CPPJIT_IMPL_VIEW_CREATOR(unsigned char);
CPPJIT_IMPL_VIEW_CREATOR(std::byte);
CPPJIT_IMPL_VIEW_CREATOR(short);
CPPJIT_IMPL_VIEW_CREATOR(unsigned short);
CPPJIT_IMPL_VIEW_CREATOR(int);
CPPJIT_IMPL_VIEW_CREATOR(unsigned int);
CPPJIT_IMPL_VIEW_CREATOR(long);
CPPJIT_IMPL_VIEW_CREATOR(unsigned long);
CPPJIT_IMPL_VIEW_CREATOR(long long);
CPPJIT_IMPL_VIEW_CREATOR(unsigned long long);
CPPJIT_IMPL_VIEW_CREATOR(float);
CPPJIT_IMPL_VIEW_CREATOR(double);
CPPJIT_IMPL_VIEW_CREATOR(long double);
CPPJIT_IMPL_VIEW_CREATOR(std::complex<float>);
CPPJIT_IMPL_VIEW_CREATOR(std::complex<double>);
CPPJIT_IMPL_VIEW_CREATOR(std::complex<int>);
CPPJIT_IMPL_VIEW_CREATOR(std::complex<long>);

PyObject* cpyrt::CreateLowLevelView(char* address, cdims_t shape) {
  LowLevelView* ll = CreateLowLevelViewT<char>(address, shape);
  CPPJIT_RET_W_CREATOR(char*, CreateLowLevelView);
}
...
PyObject* cpyrt::CreateLowLevelViewString(char** address, cdims_t shape) {
  LowLevelView* ll = CreateLowLevelViewT<char*>(address, shape, nullptr,
                                                nullptr, sizeof(char));
  CPPJIT_RET_W_CREATOR(char**, CreateLowLevelViewString);
}

PyObject* cpyrt::CreateLowLevelViewString(const char** address, cdims_t shape) {
  LowLevelView* ll = CreateLowLevelViewT<const char*>(address, shape, nullptr,
                                                      nullptr, sizeof(char));
  CPPJIT_RET_W_CREATOR(const char**, CreateLowLevelViewString);
}
...
PyObject* cpyrt::CreateLowLevelView_i8(int8_t* address, cdims_t shape) {
  ...
  CPPJIT_RET_W_CREATOR(int8_t*, CreateLowLevelView_i8);
}
