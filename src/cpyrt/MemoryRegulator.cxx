...
struct Initcpyrt_NoneType_t {
    ...
    // create a cpyrt NoneType (for references that went dodo) from NoneType
        ...
        cpyrt_NoneType.tp_richcompare = (richcmpfunc)&Initcpyrt_NoneType_t::RichCompare;
...
//- public members -----------------------------------------------------------
bool cpyrt::MemoryRegulator::RecursiveRemove(
    interop::TCppObject_t cppobj, interop::TCppScope_t klass)
{
...
//-----------------------------------------------------------------------------
bool cpyrt::MemoryRegulator::RegisterPyObject(
    CPPInstance* pyobj, interop::TCppObject_t cppobj)
{
...
//-----------------------------------------------------------------------------
bool cpyrt::MemoryRegulator::UnregisterPyObject(CPPInstance* pyobj, PyObject* pyclass)
{
// called when the proxy is garbage collected or is about delete the C++ instance
    if (!(pyobj && pyclass))
        return false;

    interop::TCppObject_t cppobj = pyobj->IsSmart() ? pyobj->GetObjectRaw() : pyobj->GetObject();
    if (!cppobj)
        return false;
...
//-----------------------------------------------------------------------------
PyObject* cpyrt::MemoryRegulator::RetrievePyObject(interop::TCppObject_t cppobj, PyObject* pyclass)
{
