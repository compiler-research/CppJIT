...
typedef std::function<std::pair<bool, bool>(cppjit::interop::TCppObject_t, cppjit::interop::TCppScope_t)> MemHook_t;
...
// callback from C++-side frameworks
    static bool RecursiveRemove(cppjit::interop::TCppObject_t cppobj, cppjit::interop::TCppScope_t klass);

// called when a new python proxy object is created
    static bool RegisterPyObject(CPPInstance* pyobj, cppjit::interop::TCppObject_t cppobj);
...
// new reference to python object matching cppobj, or 0 on failure
    static PyObject* RetrievePyObject(cppjit::interop::TCppObject_t cppobj, PyObject* pyclass);
