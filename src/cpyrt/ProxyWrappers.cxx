...
static PyObject* CreateNewCppProxyClass(interop::TCppScope_t klass, PyObject* pybases)
{
...
void AddPropertyToClass(PyObject* pyclass,
    interop::TCppScope_t scope, interop::TCppScope_t data)
{
    cpyrt::CPPDataMember* property = cpyrt::CPPDataMember_New(scope, data);
    PyObject* pname = cpyrt_PyText_InternFromString(const_cast<char*>(property->GetName().c_str()));
...
static int BuildScopeProxyDict(interop::TCppScope_t scope, PyObject* pyclass, const unsigned int flags)
{
...
// create them until needed (the same is not true for data members)
    ...
    if (isComplete) interop::GetClassMethods(scope, methods);
    ...
    // operator[]/() returning a reference type will be used for __setitem__
        ...
        if (isCall || mtName == "__getitem__") {
            const std::string& qual_return = interop::GetMethodReturnTypeAsString(method);
...
// add proxies for un-instantiated/non-overloaded templated methods
    ...
    for (auto &method : templ_methods) {
        const std::string mtCppName = interop::GetName(interop::TCppScope_t(method.data));
...
// add a pseudo-default ctor, if none defined
    if (!hasConstructor) {
        ...
        if (!isComplete) {
            ...
            defctor = new CPPIncompleteClassConstructor(scope, interop::TCppMethod_t{});
            ...
            defctor = new CPPAllPrivateClassConstructor(scope, interop::TCppMethod_t{});
        }
...
// add the methods to the class dictionary
    ...
    // one (to be picked up by the templated one as appropriate) if a template exists
        PyObject* pyname = cpyrt_PyText_FromString(const_cast<char*>(imd->first.c_str()));
        PyObject* attr = PyObject_GetItem(dct, pyname);
        Py_DECREF(pyname);
        if (TemplateProxy_Check(attr)) {
        ...
        // normal case, add a new method
            ...
            PyObject* pymname = cpyrt_PyText_InternFromString(const_cast<char*>(method->GetName().c_str()));
 ...
 // collect data members (including enums)
    ...
    // enum datamembers (this in conjunction with previously collected enums above)
        if (interop::IsEnumType(interop::GetDatamemberType(datamember)) && interop::IsEnumConstant(datamember)) {
        ...
        // provided by the class
            if (strstr(interop::GetDatamemberTypeAsString(datamember).c_str(), "(anonymous)") != 0 ||
                strstr(interop::GetDatamemberTypeAsString(datamember).c_str(), "(unnamed)")   != 0) {
...
//----------------------------------------------------------------------------
static void CollectUniqueBases(interop::TCppScope_t klass, std::deque<interop::TCppScope_t>& uqb)
{
...
// build all the bases
    if (nbases == 0) {
        ...
        for (std::deque<interop::TCppScope_t>::size_type ibase = 0; ibase < nbases; ++ibase) {
...
namespace cppjit::cpyrt {
PyObject *CppType_To_PyObject(interop::TCppType_t type, std::string name, interop::TCppScope_t parent_scope, PyObject *parent) {
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CreateScopeProxy(const std::string& name, PyObject* parent, const unsigned flags)
{
...
// retrieve C++ class (this verifies name, and is therefore done first)
    ...
    } else if (interop::TCppScope_t klass = interop::GetScope(name, parent_scope)) {
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CreateScopeProxy(interop::TCppScope_t scope, PyObject* parent, const unsigned flags)
{
// Convenience function with a lookup first through the known existing proxies.
    ...
    if (interop::IsTypedefed(scope)) {
        ...
        if ((underlying_scope) && (underlying_scope != scope)) {
            ...
            if (PyObject *result = CppType_To_PyObject(interop::GetTypeFromScope(scope), name, parent_scope, parent))
                return result;
    
            PyErr_Format(PyExc_TypeError, "\'%s\' is not a known C++ class", interop::GetScopedFinalName(scope).c_str());
            Py_XDECREF(parent);
            return nullptr;
        }
    }
    ...
    // store a ref from cppjit scope id to new python class
        if (pyscope && !(((CPPScope*)pyscope)->fFlags & CPPScope::kIsInComplete)) {
            ...
            // add to sys.modules to allow importing from this namespace
                ...
                cpyrt_PyText_AppendAndDel(&pyfullname, PyObject_GetAttr(pyscope, PyStrings::gName));
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CreateExcScopeProxy(PyObject* pyscope, PyObject* pyname, PyObject* parent)
{
    ...
    if (nbases == 0) {
        ...
        // exception classes and proper caching
            ...
            PyObject* excbase = PyObject_GetAttrString(base_parent,
                interop::GetFinalName(uqb[ibase]).c_str());
...
PyObject* cpyrt::BindCppObjectNoCast(interop::TCppObject_t address,
        interop::TCppScope_t klass, const unsigned flags)
{
...
// check whether the object to be bound is a smart pointer that needs embedding
    ...
    if (smart_type) {
        interop::TCppScope_t underlying = ((CPPSmartClass*)smart_type)->fUnderlyingType;
    ...
    //    inheritance).
        if (address && !isRef) {
            if (void* deref = interop::CallR(
                ((CPPSmartClass*)smart_type)->fDereferencer, address, 0, nullptr)) {
                interop::TCppScope_t clActual = interop::GetActualClass(underlying, deref);
                if (clActual && clActual != underlying &&
                        interop::IsSubclass(clActual, underlying) &&
                        interop::GetBaseOffset(clActual, underlying, deref, -1 /* down-cast */) == 0)
                    underlying = clActual;
            }
        }
...
//----------------------------------------------------------------------------
PyObject* cpyrt::BindCppObjectArray(
    interop::TCppObject_t address, interop::TCppScope_t klass, cdims_t dims)
{
