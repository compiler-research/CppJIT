...
//-----------------------------------------------------------------------------
bool HasAttrDirect(PyObject* pyclass, PyObject* pyname, bool mustBecpyrt = false) {
...
// are emplace_back'ed, the latter push_back'ed
    ...
    // use emplace_back to construct the vector entries one by one
        ...
        // if the value_type is a vector, then allow for initialization from sequences
            if (interop::GetTypeAsString(PyLong_AsVoidPtr(vtype)).rfind("std::vector", 0) != std::string::npos)
...
// a buffer here is slow, so use insert() instead
    if (PyTuple_GET_SIZE(args) == 1) {
        PyObject* fi = PyTuple_GET_ITEM(args, 0);
        if (PyObject_CheckBuffer(fi) && !(cpyrt_PyText_Check(fi) || PyBytes_Check(fi))) {
    ...
    if (pyvalue_type) {
        ...
        if (PyLong_Check(pyvalue_type)) {
            ...
            // is more useful to treat it polymorphically by allowing auto-downcasts)
                const std::string& clean_type = TypeManip::clean_type(interop::GetTypeAsString(value_type), false, false);
                interop::TCppScope_t c = interop::GetScope(clean_type);
                if (c && TypeManip::compound(interop::GetTypeAsString(value_type)) == "*") {
                    vi->vi_klass = c;
            ...
            if (vi->vi_klass) {
                ...
                if (!vi->vi_flags) {
                    value_type = interop::ResolveType(value_type);
                    if (interop::GetTypeAsString(value_type).back() != '*')     // meaning, object stored by-value
                        vi->vi_flags = vectoriterobject::kNeedLifeLine;
                }
            } else
                vi->vi_converter = cppjit::cpyrt::CreateConverter(value_type);
            if (!vi->vi_stride) vi->vi_stride = interop::SizeOfType(value_type);

        } else if (CPPScope_Check(pyvalue_type)) {
            vi->vi_klass = ((CPPClass*)pyvalue_type)->fCppType;
            vi->vi_converter = nullptr;
            if (!vi->vi_stride) vi->vi_stride = interop::SizeOf(vi->vi_klass);
...
// Implement python's __getitem__ for std::vector<>s.
    if (PySlice_Check(index)) {
        ...
        PySlice_GetIndices((cpyrt_PySliceCast)index, PyObject_Length((PyObject*)self), &start, &stop, &step);
...
// the compiler: treat it special here as well
    ...
    if (PySlice_Check(idx)) {
        ...
        Py_ssize_t start, stop, step;
        PySlice_GetIndices((cpyrt_PySliceCast)idx, PyObject_Length((PyObject*)self), &start, &stop, &step);
        const Py_ssize_t nlen = PySequence_Size((PyObject*)self);
        if (!AdjustSlice(nlen, start, stop, step))
            return nseq;
...
static inline
PyObject* cpyrt_PyString_FromCppString(std::string_view s, bool native=true) {
    if (native)
...
static inline
PyObject* cpyrt_PyString_FromCppString(std::wstring_view s, bool native=true) {
...
{                                                                            \
    if (cpyrt::CPPInstance_Check(self)) {                                 \
        ...
        if (obj) return cpyrt_PyString_FromCppString(*obj, native);        \
    }                                                                        \
...
// overload that takes a string
    ...
    if (2 <= PyTuple_GET_SIZE(args) && cpyrt_PyText_Check(PyTuple_GET_ITEM(args, 0))) {
...
// and algorithms
    if (HasAttrDirect(pyclass, PyStrings::gDeref) && !interop::IsSmartPtr(klass->fCppType))
        Utility::AddToClass(pyclass, "__getattr__", (PyCFunction)DeRefGetAttr, METH_O);
    else if (HasAttrDirect(pyclass, PyStrings::gFollow) && !interop::IsSmartPtr(klass->fCppType))
...
// called "size".
    ...
    if (!IsTemplatedSTLClass(name, "vector")  &&      // vector is dealt with below
        ...
        // obtain the name of the return type
            const auto& methods = interop::GetMethodsFromName(klass->fCppType, "begin");
            if (!methods.empty()) {
            ...
            // types to add the "next" method on use
                ...
                // won't work: the return type needs to understand the iterator protocol)
                    ...
                    if (resolved.back() == '*' && interop::IsBuiltin(resolved.substr(0, resolved.size()-1))) {
...
// lookups in the global and/or class namespace
    ...
    if (interop::IsAggregate(((CPPClass*)pyclass)->fCppType) && name.compare(0, 5, "std::", 5) != 0) {
    // create a pseudo-constructor to allow initializer-style object creation
        ...
        if (!datamems.empty()) {
            ...
            for (auto data : datamems) {
                ...
                interop::TCppType_t datammember_type =
                    interop::GetDatamemberType(data);
                ...
                if (res.rfind(']') == std::string::npos && res.rfind(')') == std::string::npos) {
                    ...
                    if ((!cpd.empty() && cpd.back() == '*') || interop::IsBuiltin(res_clean))
                        arg_defaults.push_back("0");
                    else {
                        interop::TCppScope_t klsid = interop::GetScope(res_clean);
                        if (interop::IsDefaultConstructable(klsid)) arg_defaults.push_back(res_clean+"{}");
                    }
                } else {
            ...
            if (codegen_ok && !arg_types.empty()) {
                ...
                if (interop::Compile(initdef.str(), true /* silent */)) {
                    ...
                    const auto& methods = interop::GetMethodsFromName(cis, "init_" + rname);
                    if (methods.size()) {
...
//- class name based pythonization -------------------------------------------
    ...
    // std::vector<bool> is a special case in C++
        if (!sVectorBoolTypeID) sVectorBoolTypeID = interop::GetScope("std::vector<bool>");
        ...
        // helpers for iteration
            interop::TCppType_t value_type = interop::GetTypeFromScope(interop::GetNamed("value_type", scope));
            ...
            if (vtype) {    // actually resolved?
                ...
                pyvalue_type = PyUnicode_FromString(interop::GetTypeAsString(vtype).c_str());
    ...
    if (IsTemplatedSTLClass(name, "shared_ptr") || IsTemplatedSTLClass(name, "unique_ptr")) {
        ...
        Utility::AddToClass(pyclass, CPPJIT__next__, (PyCFunction)STLIterNext, METH_NOARGS);
