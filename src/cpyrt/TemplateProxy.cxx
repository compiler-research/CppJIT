...
PyObject* TemplateProxy::Instantiate(const std::string& fname,
    cpyrt_PyArgs_t args, size_t nargsf, Utility::ArgPreference pref, int* pcnt)
{
...
// adjust arguments for self if this is a rebound global function
    ...
    if (!isNS && cpyrt_PyArgs_GET_SIZE(args, nargsf) && \
            (!fSelf ||
            (fSelf == Py_None && !interop::IsStaticTemplate(((CPPScope*)fTI->fPyClass)->fCppType, fname)))) {
        args   += 1;
        nargsf -= 1;
    }
...
// the following causes instantiation as necessary
    ...
    interop::TCppMethod_t cppmeth = interop::GetMethodTemplate(scope, fname, proto);
    ...
    // can add already existing overloads to the set of methods.
        ...
        std::string resname = interop::GetFullName(interop::TCppScope_t(cppmeth.data));
    ...
    // the argument types. If it did, replace with vector and lookup anew.
        if (resname.find("initializer_list") != std::string::npos) {
            ...
            interop::TCppMethod_t m2 = interop::GetMethodTemplate(scope, fname, proto);
            if (m2 && m2 != cppmeth) {
...
static inline PyObject* CallMethodImp(TemplateProxy* pytmpl, PyObject*& pymeth,
    cpyrt_PyArgs_t args, size_t nargsf, PyObject* kwds, bool impOK, uint64_t sighash)
{
...
// case 1: explicit template previously selected through subscript
    ...
    // instantiate explicitly
        PyObject* pyfullname = cpyrt_PyText_FromString(pytmpl->fTI->fCppName.c_str());
    ...
    // not cached or failed call; try instantiation
        pymeth = pytmpl->Instantiate(
            cpyrt_PyText_AsString(pyfullname), args, nargsf, Utility::kNone);
        if (pymeth) {
    ...
    // no drop through if failed (if implicit was desired, don't provide template args)
        ...
        PyObject* topmsg = cpyrt_PyText_FromFormat(
            "Could not find \"%s\" (set cppjit.set_debug() for C++ errors):", cpyrt_PyText_AsString(pyfullname));
        Py_DECREF(pyfullname);
...
// error reporting is fraud, given the numerous steps taken, but more details seems better
    if (!errors.empty()) {
        PyObject* topmsg = cpyrt_PyText_FromString("Template method resolution failed:");
        Utility::SetDetailedException(std::move(errors), topmsg /* steals */, PyExc_TypeError /* default error */);
    } else {
...
PyTypeObject TemplateProxy_Type = {
    ...
    (char*)"cppjit.TemplateProxy",      // tp_name
