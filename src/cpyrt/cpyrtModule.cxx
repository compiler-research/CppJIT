...
// the lookup, so it can no longer be shimmed and "from cppjit.interactive import *"
...
#define CPYRT_GET_DICT_LOOKUP(mp)                                          \
...
Py_ssize_t cpyrtLookDictString(PyDictObject* mp, PyObject* key,
    Py_hash_t hash, PyObject*** value_addr, Py_ssize_t* hashpos)
{
...
// Helper to get the address (address-of-address) of various object proxy types.
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwds, const_cast<char*>("O|O!b"), GCIA_kwlist,
            &pyobj, &cpyrt_PyText_Type, &pyname, &byref)) {
        ...
        if (CPPInstance_Check(pyobj)) {
            ...
            // locate property proxy for offset info
                ...
                PyErr_Format(PyExc_TypeError,
                    "%s is not a valid data member", cpyrt_PyText_AsString(pyname));
                return nullptr;
            }
...
{
// Return object proxy address as a value (cppjit-style), or the same for an array.
    ...
    if (!PyErr_Occurred()) {
        if (PyTuple_CheckExact(args) && PyTuple_GET_SIZE(args)) {
            ...
            if (str && cpyrt_PyText_Check(str))
                PyErr_Format(PyExc_TypeError, "unknown object %s", cpyrt_PyText_AsString(str));
            else
...
// convert 2nd argument first (used for both pointer value and instance cases)
    ...
    if (!cast_type && arg1) {
        cast_type = (interop::TCppScope_t)interop::GetScope(cpyrt_PyText_AsString(arg1));
        Py_DECREF(arg1);
    }
...
// next, convert the first argument, some pointer value or a pre-existing instance
    ...
    // offset, erase if from any caches, and update the pointer and type
        ...
        interop::TCppScope_t cur_type = arg0_pyobj->ObjectIsA(false /* check_smart */);
        ...
        interop::TCppObject_t address = (interop::TCppObject_t)arg0_pyobj->GetObject();
        ptrdiff_t offset = interop::GetBaseOffset(derived, base, address, direction);
    ...
    // surprise" as most likely only the cast object will be retained
        ...
        // ordinary C++ class
            PyObject* pyobj = BindCppObjectNoCast(
                interop::TCppObject_t((void*)((intptr_t)address.data + offset)), cast_type, owns ? CPPInstance::kIsOwner : 0);
...
// Remove a previously registered pythonizor from the given scope.
    ...
    if (!PyCallable_Check(pythonizor)) {
        ...
        PyErr_Format(PyExc_TypeError,
            "given \'%s\' object is not callable", cpyrt_PyText_AsString(pystr));
        Py_DECREF(pystr);
        return nullptr;
    }
...
// Add a type reducer to map type2 to type2 on function returns.
    ...
    interop::TCppType_t reducable_type = interop::GetTypeFromScope(interop::GetScope(reducable));
    interop::TCppType_t reduced_type = interop::GetTypeFromScope(interop::GetScope(reduced));
...
static struct PyModuleDef moduledef = {
    ...
    gcpyrtMethods,
    nullptr,
    cpycppjitmodule_traverse,
    cpycppjitmodule_clear,
    nullptr
};
...
// C++-specific exceptions
    PyObject* cppfatal = PyErr_NewException((char*)"cppjit.ll.FatalError", nullptr, nullptr);
    PyModule_AddObject(gThisModule, (char*)"FatalError", cppfatal);

    gBusException  = PyErr_NewException((char*)"cppjit.ll.BusError", cppfatal, nullptr);
    PyModule_AddObject(gThisModule, (char*)"BusError", gBusException);
    gSegvException = PyErr_NewException((char*)"cppjit.ll.SegmentationViolation", cppfatal, nullptr);
    PyModule_AddObject(gThisModule, (char*)"SegmentationViolation", gSegvException);
    gIllException  = PyErr_NewException((char*)"cppjit.ll.IllegalInstruction", cppfatal, nullptr);
    PyModule_AddObject(gThisModule, (char*)"IllegalInstruction", gIllException);
    gAbrtException = PyErr_NewException((char*)"cppjit.ll.AbortSignal", cppfatal, nullptr);
